...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [animationsEnabled, setAnimationsState] = useState<boolean>(getInitialAnimations);
  ...
  const toggleAnimations = useCallback(() => {
    setAnimationsState(prev => {
      const newVal = !prev;
      localStorage.setItem('animations', String(newVal));
      return newVal;
    });
  }, []);
  ...
  const value = useMemo<ThemeContextType>(() => ({
    ...
    toggleAnimations,
    setAnimations,
  }), [theme, toggleTheme, setTheme, animationsEnabled, toggleAnimations, setAnimations]);
