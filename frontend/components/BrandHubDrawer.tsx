              ...
              {colorEntries.slice(0, 8).map((c, i) => (
                <div key={i} className="flex flex-col items-center gap-1 group cursor-default">
                  <div
                    ...
                    style={{ backgroundColor: c.hex, borderColor: isDarkMode ? 'var(--border-subtle)' : 'rgba(0,0,0,0.08)' }}
...
export function DNAIcon({ size = 20, animated = false, isDark = false }: { size?: number; animated?: boolean; isDark?: boolean }) {
  ...
  const accentLight = isDark ? '#6BA3FF' : '#374151';
  const accentDark = isDark ? '#4D8FFF' : '#1D6AE5';
  const glow = isDark ? 'rgba(59,130,246,0.45)' : 'rgba(55,65,81,0.25)';
  const glowLg = isDark ? 'rgba(59,130,246,0.2)' : 'rgba(55,65,81,0.1)';
        ...
        <svg viewBox="0 0 24 24" width={size} height={size} fill="none">
          <defs>
            <linearGradient id={`${uid}-a`} x1="0%" y1="0%" x2="0%" y2="100%">
              ...
              <stop offset="100%" stopColor={isDark ? '#3b82f6' : '#1558C0'} />
            </linearGradient>
            <linearGradient id={`${uid}-b`} x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stopColor={isDark ? '#4D8FFF' : '#374151'} />
...
function ColorSwatch({ color, isDark }: { color: string; isDark: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="w-5 h-5 rounded-md border" style={{ backgroundColor: hex, borderColor: isDark ? 'var(--glass-bg-light)' : 'rgba(0,0,0,0.1)' }} />
