...
interface ComputerDesktopProps {
  ...
  /** Goal queued before this component mounted. Consumed on first render. */
  pendingGoal?: { goal: string; workspaceId?: string } | null;
  /** Called after pendingGoal is consumed so parent can clear it. */
  onPendingGoalConsumed?: () => void;
}

export const ComputerDesktop = forwardRef<ComputerDesktopHandle, ComputerDesktopProps>(function ComputerDesktop({ sessionId, computerId, humanControl, onHumanControlChange, pendingGoal, onPendingGoalConsumed }, ref) {
  ...
  const queuedGoalsRef = useRef<Array<{ goal: string; workspaceId?: string }> | null>(null);
  ...
  // -- Launch computer agent (shared by bus handler + pendingGoal) --
  const agentStartedAt = useRef<number>(0);
  const launchAgent = useCallback((goal: string, workspaceId?: string) => {
    console.log('[ComputerDesktop] launchAgent called:', { goal: goal.slice(0, 60), agentRunning: agentRunningRef.current, hasDesktop: !!desktopRef.current });
    // Auto-reset stuck agent lock after 3 minutes (safety net)
    if (agentRunningRef.current && Date.now() - agentStartedAt.current > 3 * 60 * 1000) {
      console.warn('[ComputerDesktop] agentRunningRef stuck for 3min, force-resetting');
      agentRunningRef.current = false;
    }
    if (agentRunningRef.current) {
      console.warn('[ComputerDesktop] BLOCKED: agent already running');
      desktopBus.emit({ type: 'agent_status', phase: 'executing', message: 'Agent is working on something... queuing your request for next' });
      // Queue this goal to run after current task completes
      if (!queuedGoalsRef.current) queuedGoalsRef.current = [];
      queuedGoalsRef.current.push({ goal, workspaceId });
      return;
    }
    if (!desktopRef.current) {
      console.warn('[ComputerDesktop] BLOCKED: desktopRef is null');
      desktopBus.emit({ type: 'agent_status', phase: 'error', message: 'Desktop element not available' });
      return;
    }
    agentRunningRef.current = true;
    agentStartedAt.current = Date.now();
    console.log('[ComputerDesktop] launching runComputerAgent...');
    runComputerAgent(goal, desktopRef.current, {
      onStatus: (msg: string) => {
        console.log('[ComputerDesktop]', msg);
        if (msg.includes('[Agent]') || msg.includes('error') || msg.includes('fail') || msg.includes('Starting') || msg.includes('Browser')) {
          desktopBus.emit({ type: 'agent_status', phase: 'executing', message: msg.replace(/^\[.*?\]\s*/, '') });
        }
      },
      onScreenshot: (base64: string) => {
        desktopBus.emit({ type: 'computer_screenshot', screenshot: base64 });
      },
      ...
      if (queuedGoalsRef.current && queuedGoalsRef.current.length > 0) {
        const nextGoal = queuedGoalsRef.current.shift();
        if (nextGoal) {
          setTimeout(() => launchAgent(nextGoal.goal, nextGoal.workspaceId), 100);
        }
      }
    });
  }, []);
    ...
    return desktopBus.subscribe(event => {
      switch (event.type) {
        ...
        case 'browser_stream_stop':
          ...
          break;
        case 'run_goal':
          launchAgent(event.goal, event.workspaceId);
          break;
        case 'ask_user':
  ...
  // -- Consume pending goal passed as prop (fires once on mount when goal was queued before mount)
  const pendingGoalConsumed = useRef(false);
  useEffect(() => {
    if (!pendingGoal || pendingGoalConsumed.current) return;
    pendingGoalConsumed.current = true;
    // Wait a tick so desktopRef.current is assigned after render
    const timer = setTimeout(() => {
      if (!desktopRef.current || agentRunningRef.current) {
        console.warn('[ComputerDesktop] pendingGoal skipped:', { hasDesktop: !!desktopRef.current, agentRunning: agentRunningRef.current });
        return;
      }
      console.log('[ComputerDesktop] pendingGoal launching:', pendingGoal.goal.slice(0, 60));
      launchAgent(pendingGoal.goal, pendingGoal.workspaceId);
      onPendingGoalConsumed?.();
    }, 80);
    return () => {
      clearTimeout(timer);
      // Reset so StrictMode remount can re-fire (mount → unmount clears timer → remount must retry)
      pendingGoalConsumed.current = false;
    };
  ...
  // Capture desktop screenshot on mount (after brief delay for DOM paint) so mini-screen has content
  useEffect(() => {
    const timer = setTimeout(async () => {
      if (!desktopRef.current) return;
      try {
        const { default: html2canvas } = await import('html2canvas');
        const canvas = await html2canvas(desktopRef.current, { scale: 0.5, useCORS: true, logging: false, allowTaint: true,
          ignoreElements: (el) => { try { const s = window.getComputedStyle(el); if (s.backgroundImage?.includes('gradient') && el.getBoundingClientRect().width === 0) return true; } catch {} return false; },
        });
    ...
    <ErrorBoundary>
      <div ref={desktopRef} data-desktop-root className="w-full h-full flex flex-col" style={{ position: 'relative', pointerEvents: humanControl ? 'auto' : 'none' }}>
        ...
        <div style={{
          ...
          background: 'var(--bg-secondary)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--glass-border-light)',
          flexShrink: 0,
        }}>
          <span style={{ fontSize: 10, fontWeight: 500, color: 'var(--text-muted)' }}>{time}</span>
        </div>
        ...
        <div className="flex-1 relative" style={{ minHeight: 0 }}>
          ...
          {/* Take Control button — only visible when AI is in control */}
          {!humanControl && (
            <motion.button
              ...
              onClick={() => onHumanControlChange?.(true)}
              style={{
                position: 'absolute',
                top: 16,
                right: 16,
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'var(--glass-border-medium)';
        ...
        <div style={{
          ...
          <motion.div
            ...
            style={{
              ...
              border: '1px solid var(--glass-border-light)', boxShadow: '0 4px 16px rgba(0,0,0,0.25)',
            }}
          >
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div style={{ position: 'relative' }}>
        ...
        {isOpen && (
          <div style={{ position: 'absolute', bottom: -3, left: '50%', transform: 'translateX(-50%)', width: 3, height: 3, borderRadius: '50%', background: 'var(--text-primary)' }} />
        )}
      </div>
      <div style={{ fontSize: 9, color: 'var(--text-muted)', textAlign: 'center', marginTop: 1 }}>{label}</div>
    </div>
  );
}
