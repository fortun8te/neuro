...
function Preview({ node }: { node: DNode }) {
  ...
  const ps: React.CSSProperties = { borderTop: '1px solid var(--border-subtle)', background: 'var(--bg-secondary)', flexShrink: 0, overflow: 'auto', maxHeight: 260, minHeight: 60 };

  if (node.type === 'folder') {
    const ch = vfs.listFolder(node.vfsPath);
    return (<div style={{ ...ps, padding: '10px 14px' }}><div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>{node.name}</div><div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>Folder -- {ch.length} item{ch.length !== 1 ? 's' : ''}</div><div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>{node.vfsPath}</div></div>);
  }
  if (isImg && data) {
    const src = data.startsWith('data:') ? data : `data:${node.mimeType || 'image/jpeg'};base64,${data}`;
    return (<div style={ps}><div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderBottom: '1px solid var(--glass-border-light)', fontSize: 11, color: 'var(--text-secondary)' }}><span>{node.name}</span><span style={{ flex: 1 }} /><button onClick={() => setZoom(z => Math.max(0.25, z - 0.25))} style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 13 }}>-</button><span style={{ fontSize: 10, color: 'var(--text-secondary)', minWidth: 28, textAlign: 'center' }}>{Math.round(zoom * 100)}%</span><button onClick={() => setZoom(z => Math.min(4, z + 0.25))} style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 13 }}>+</button></div><div style={{ overflow: 'auto', padding: 8, display: 'flex', justifyContent: 'center' }}><img src={src} alt={node.name} style={{ maxWidth: `${zoom * 100}%`, height: 'auto', borderRadius: 4, imageRendering: zoom >= 2 ? 'pixelated' : 'auto' }} /></div></div>);
  }
  if (isPdf) {
    const blob = data.startsWith('blob:');
    return (<div style={ps}><div style={{ padding: '4px 10px', borderBottom: '1px solid var(--glass-border-light)', fontSize: 11, color: 'var(--text-secondary)' }}>{node.name}</div>{blob ? <iframe src={data} title={node.name} style={{ width: '100%', height: 220, border: 'none', background: '#222' }} /> : <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>PDF -- {node.size || 'unknown size'}</div>}</div>);
  }
  if (isTxt && data) {
    return (<div style={ps}><div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderBottom: '1px solid var(--glass-border-light)', fontSize: 11, color: 'var(--text-secondary)' }}>{node.name}{canEdit && <button onClick={save} style={{ marginLeft: 'auto', background: 'rgba(234,88,12,0.2)', border: '1px solid rgba(234,88,12,0.4)', borderRadius: 4, color: '#5B9BF8', fontSize: 10, padding: '2px 6px', cursor: 'pointer' }}>Save</button>}</div><textarea value={canEdit ? edited : data} onChange={canEdit ? e => setEdited(e.target.value) : undefined} readOnly={!canEdit} spellCheck={false} style={{ width: '100%', height: 180, background: 'rgba(0,0,0,0.3)', color: 'var(--text-primary)', border: 'none', outline: 'none', padding: 10, fontSize: 11, fontFamily: '"SF Mono",monospace', resize: 'none', lineHeight: 1.5, boxSizing: 'border-box' }} /></div>);
  }
  return (<div style={{ ...ps, padding: '10px 14px' }}><div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>{node.name}</div><div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 2 }}>{ext ? ext.toUpperCase() + ' File' : 'File'}{node.size && ` -- ${node.size}`}</div><div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>{node.vfsPath}</div></div>);
}
...
function RenameInput({ initial, onCommit, onCancel }: { initial: string; onCommit: (n: string) => void; onCancel: () => void }) {
  ...
  return <input ref={ref} value={val} onChange={e => setVal(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); onCommit(val.trim() || initial); } if (e.key === 'Escape') { e.preventDefault(); onCancel(); } }} onBlur={() => onCommit(val.trim() || initial)} onClick={e => e.stopPropagation()} style={{ fontSize: 11, color: '#fff', background: 'rgba(234,88,12,0.3)', border: '1px solid rgba(234,88,12,0.8)', borderRadius: 3, outline: 'none', padding: '0 3px', width: '100%', maxWidth: 76, textAlign: 'center', ...S, boxSizing: 'border-box' }} />;
}
...
function CtxPopup({ menu, onClose, onAction }: { menu: CtxMenu; onClose: () => void; onAction: (a: string) => void }) {
  ...
  return (<div ref={ref} onMouseDown={e => e.stopPropagation()} style={{ position: 'fixed', left: menu.x, top: menu.y, zIndex: 999, background: 'rgba(28,28,34,0.98)', border: '1px solid var(--glass-border-medium)', borderRadius: 8, padding: '4px 0', minWidth: 150, boxShadow: '0 8px 32px rgba(0,0,0,0.7)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>{items.map((it, i) => (<button key={i} onClick={() => { onAction(it.act); onClose(); }} style={{ display: 'block', width: '100%', padding: '6px 14px', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', fontSize: 12.5, color: it.danger ? '#f87171' : 'var(--text-primary)' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--glass-bg-light)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'none'; }}>{it.label}</button>))}</div>);
}
  ...
  return (
    <motion.div ref={windowRef} initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.92 }} transition={{ duration: 0.2, ease: [0.25, 0.46, 0.45, 0.94] }} onMouseDownCapture={onFocus} style={{ position: 'absolute', ...(pos ? { left: pos.x, top: pos.y, transform: 'none' } : { left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }), width: 860, height: 540, maxWidth: 'calc(100% - 8px)', maxHeight: 'calc(100% - 8px)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', zIndex: zIndex ?? 200, pointerEvents: 'auto', background: 'var(--bg-primary)', boxShadow: '0 4px 24px rgba(0,0,0,0.4)', border: 'none', ...S, userSelect: 'none' }}>
      ...
      <div ref={kbRef} tabIndex={0} onKeyDown={onKb} style={{ display: 'contents', outline: 'none' }}>
        {/* Title bar */}
        <div onMouseDown={onTitleBarMouseDown} style={{ height: 36, display: 'flex', alignItems: 'center', paddingLeft: 14, paddingRight: 14, background: 'var(--bg-secondary)', borderBottom: '1px solid var(--glass-border-light)', flexShrink: 0, cursor: isDragging ? 'grabbing' : 'grab', position: 'relative' }}>
          <button onClick={onClose} style={{ width: 12, height: 12, borderRadius: '50%', background: 'var(--glass-border-medium)', border: 'none', cursor: 'pointer', padding: 0 }} onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,95,87,0.7)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'var(--glass-border-medium)'; }} title="Close" />
          <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', letterSpacing: -0.2 }}>Files</div>
        </div>
        {/* Toolbar */}
        <div style={{ height: 40, display: 'flex', alignItems: 'center', padding: '0 10px', gap: 6, background: 'var(--bg-secondary)', borderBottom: '1px solid var(--glass-border-light)', flexShrink: 0 }}>
          <button onClick={goBack} disabled={!canBack} style={{ background: 'none', border: 'none', padding: '4px 5px', borderRadius: 5, color: canBack ? 'var(--text-primary)' : 'var(--text-muted)', cursor: canBack ? 'pointer' : 'default', display: 'flex' }}>{chev(true)}</button>
          <button onClick={goFwd} disabled={!canFwd} style={{ background: 'none', border: 'none', padding: '4px 5px', borderRadius: 5, color: canFwd ? 'var(--text-primary)' : 'var(--text-muted)', cursor: canFwd ? 'pointer' : 'default', display: 'flex' }}>{chev(false)}</button>
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 3, overflow: 'hidden', minWidth: 0 }}>
            {crumbs.map((bc, i, a) => (<div key={bc.path} style={{ display: 'flex', alignItems: 'center', gap: 3, flexShrink: i < a.length - 1 ? 1 : 0 }}>{i > 0 && chev(false)}<button onClick={() => { if (i < a.length - 1) navTo(bc.path); }} style={{ background: 'none', border: 'none', padding: '2px 4px', borderRadius: 4, fontSize: 12, cursor: i < a.length - 1 ? 'pointer' : 'default', color: i === a.length - 1 ? 'var(--text-primary)' : 'var(--text-secondary)', fontWeight: i === a.length - 1 ? 600 : 400, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 120, textTransform: i === 0 ? 'capitalize' : 'none' }}>{bc.label}</button></div>))}
          </div>
          <div style={{ display: 'flex', gap: 1, background: 'var(--glass-bg-light)', borderRadius: 6, padding: 2 }}>
            {(['grid', 'list'] as const).map(m => (<button key={m} onClick={() => setView(m)} style={{ background: view === m ? 'var(--glass-border-medium)' : 'none', border: 'none', padding: '3px 7px', borderRadius: 5, cursor: 'pointer', color: view === m ? 'var(--text-primary)' : 'var(--text-secondary)', fontSize: 10 }}>{m === 'grid' ? 'Grid' : 'List'}</button>))}
          </div>
          <button onClick={newFolder} title="New Folder" style={{ background: 'none', border: 'none', padding: '4px 6px', borderRadius: 5, cursor: 'pointer', color: 'var(--text-secondary)', fontSize: 11 }} onMouseEnter={e => { e.currentTarget.style.color = 'var(--text-primary)'; }} onMouseLeave={e => { e.currentTarget.style.color = 'var(--text-secondary)'; }}>+ Folder</button>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search" style={{ width: 140, paddingLeft: 8, paddingRight: 8, height: 26, borderRadius: 6, border: '1px solid var(--glass-bg-light)', background: 'var(--glass-bg-light)', color: 'var(--text-primary)', fontSize: 12, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
        </div>
        {/* Body */}
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          <div style={{ width: sbW, background: 'var(--bg-tertiary)', borderRight: '1px solid var(--glass-border-light)', flexShrink: 0, overflowY: 'auto', padding: '8px 4px' }}>
          ...
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            {view === 'list' && (<div style={{ display: 'flex', alignItems: 'center', padding: '0 12px', height: 26, borderBottom: '1px solid var(--glass-border-light)', background: 'rgba(18,18,24,0.95)' }}><span style={{ flex: 1, fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Name</span><span style={{ width: 90, fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textAlign: 'right', flexShrink: 0 }}>Modified</span><span style={{ width: 60, fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textAlign: 'right', flexShrink: 0 }}>Size</span></div>)}
            <div style={{ flex: 1, overflowY: 'auto', padding: view === 'grid' ? 12 : 0 }} onClick={() => { setSelId(null); setCtx(null); }}>
              {items.length === 0 ? (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--text-muted)', fontSize: 13 }}>Empty</div>) : view === 'grid' ? (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}><AnimatePresence>{items.map(it => (
                  <motion.div key={it.id} whileTap={{ scale: 0.96 }} onClick={e => { e.stopPropagation(); setSelId(it.id); }} onDoubleClick={renId === it.id ? undefined : () => openItem(it)} onContextMenu={e => { e.preventDefault(); e.stopPropagation(); setSelId(it.id); setCtx({ x: e.clientX, y: e.clientY, nodeId: it.id, nodeType: it.type, vfsPath: it.vfsPath }); }} className="flex flex-col items-center gap-1 p-2 rounded-lg cursor-default select-none" style={{ background: selId === it.id ? 'rgba(234,88,12,0.25)' : 'transparent', minWidth: 80, maxWidth: 80 }}>
                    <div style={{ width: 42, height: 42, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28 }}>{it.type === 'folder' ? '\uD83D\uDCC1' : '\uD83D\uDCC4'}</div>
                    {renId === it.id ? <RenameInput initial={it.name} onCommit={n => commitRen(it.id, n)} onCancel={() => setRenId(null)} /> : <span style={{ fontSize: 11, color: selId === it.id ? 'var(--accent-blue)' : 'var(--text-primary)', textAlign: 'center', maxWidth: 76, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as unknown as undefined, lineHeight: 1.3 }}>{it.name}</span>}
                  </motion.div>
                ))}</AnimatePresence></div>
              ) : (<div>{items.map((it, i) => (
                <div key={it.id} onClick={e => { e.stopPropagation(); setSelId(it.id); }} onDoubleClick={renId === it.id ? undefined : () => openItem(it)} onContextMenu={e => { e.preventDefault(); e.stopPropagation(); setSelId(it.id); setCtx({ x: e.clientX, y: e.clientY, nodeId: it.id, nodeType: it.type, vfsPath: it.vfsPath }); }} className="flex items-center gap-2 px-3 cursor-default select-none" style={{ background: selId === it.id ? 'rgba(234,88,12,0.25)' : 'transparent', borderTop: i === 0 ? 'none' : '1px solid var(--glass-border-light)', height: 28 }}>
                  <span style={{ fontSize: 14, width: 16, textAlign: 'center', flexShrink: 0 }}>{it.type === 'folder' ? '\uD83D\uDCC1' : '\uD83D\uDCC4'}</span>
                  {renId === it.id ? <RenameInput initial={it.name} onCommit={n => commitRen(it.id, n)} onCancel={() => setRenId(null)} /> : <span style={{ flex: 1, fontSize: 12, color: selId === it.id ? 'var(--accent-blue)' : 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{it.name}</span>}
                  {renId !== it.id && <><span style={{ fontSize: 11, color: 'var(--text-muted)', width: 90, textAlign: 'right', flexShrink: 0, whiteSpace: 'nowrap' }}>{it.modified}</span><span style={{ fontSize: 11, color: 'var(--text-muted)', width: 60, textAlign: 'right', flexShrink: 0 }}>{it.size ?? '--'}</span></>}
                </div>
              ))}</div>)}
            </div>
        ...
        {/* Status bar */}
        <div style={{ height: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-tertiary)', borderTop: '1px solid var(--glass-border-light)', fontSize: 11, color: 'var(--text-muted)', flexShrink: 0 }}>
