...
function ServiceHealthDot() {
  ...
  useEffect(() => {
    const check = async () => {
      try {
        const [ollamaOk, wayfarerOk] = await Promise.all([
          fetch(INFRASTRUCTURE.ollamaUrl + '/api/tags', { signal: AbortSignal.timeout(3000) }).then(r => r.ok).catch(() => false),
          fetch(INFRASTRUCTURE.wayfarerUrl + '/health', { signal: AbortSignal.timeout(3000) }).then(r => r.ok).catch(() => false),
        ]);
        setHealth(ollamaOk && wayfarerOk ? 'ok' : ollamaOk ? 'degraded' : 'down');
      } catch {
        setHealth('down');
      }
    };
  ...
  const color = health === 'ok' ? '#22c55e' : health === 'degraded' ? '#f59e0b' : '#ef4444';
  const title = health === 'ok' ? 'All services online' : health === 'degraded' ? 'Some services unavailable' : 'Services offline';
...
function LeftPanel({
  ...
  // Research presets removed — system auto-configures
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        {(p?.brand?.name || campaign.brand) && (
          ...
          >
            ...
            <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={isDark ? 'var(--text-muted)' : '#d4d4d8'} strokeWidth="2.5" strokeLinecap="round">
        ...
        <div className={`px-3 py-3 border-t ${isDark ? 'border-white/[0.08]' : 'border-black/[0.06]'}`}>
          {isRunning ? (
            ...
            <button
              ...
              style={{ background: 'rgba(234,88,12,0.9)', boxShadow: '0 2px 12px rgba(234,88,12,0.25)' }}
            >
              <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-8">
      <div className="text-center space-y-5">
        ...
        <button
          ...
          style={{
            background: isRunning ? (isDarkMode ? 'rgba(234,88,12,0.1)' : 'rgba(234,88,12,0.06)') : 'rgba(234,88,12,0.9)',
            color: isRunning ? (isDarkMode ? 'var(--text-secondary)' : 'var(--text-primary)') : '#fff',
            boxShadow: isRunning ? 'none' : '0 2px 12px rgba(234,88,12,0.25)',
          }}
        >
