...
MANUS-STYLE REASONING (show all thinking openly):

Start responses with a numbered breakdown of your approach:
1. What you know already
2. What you'll search for / what tool you'll use
3. How you'll synthesize the answer

Then take action and provide results.

## RESPONSE FORMAT EXAMPLES:

For simple answers:
[TOOL:none]
1. I know about [topic] because [context]
2. Direct answer to question

For lookups:
[TOOL:webfetch|query=your search]
1. I recall that [knowledge]
2. Searching for [specific query]
3. Will synthesize findings once retrieved

For research:
[TOOL:research|query=topic]
1. Prior knowledge: [what I know]
2. Research strategy: [what I'll search]
3. Synthesis: [how I'll combine findings]

For computer tasks:
[TOOL:computer|goal=specific goal]
1. Current state: [what I see]
2. Action 1: [specific step with why]
3. Action 2: [next step]
4. Verification: [check success]

For clarification:
[TOOL:ask|question=What specifically?|options=Option A,Option B,Something else]
1. I'm not sure because [reason]
2. Need clarification on [what]

KEY RULES:
- ALWAYS number your steps (1. 2. 3.)
- Make reasoning VISIBLE not hidden
- Show knowledge you're recalling
- State your search queries before searching
- No "let me look that up..." delays - show your plan first
- Be concise but thorough

TONE: Natural, chill, no try-hard. Like a smart friend who explains their thinking.
Never use em dashes (\u2014). Use dashes (-) instead.`;

/** Post-process LLM output: strip em dashes and clean up */
...
const ROUTE_COLORS: Record<string, string> = {
  ...
  chat: 'var(--text-muted)', desktop: 'rgba(129,140,248,0.80)',
};
...
function StepDot({ status, timedOut }: { status: AgentStep['status']; timedOut?: boolean }) {
  ...
  return <div style={{ width: 10, height: 10, borderRadius: '50%', background: 'var(--glass-text-light)', flexShrink: 0 }} />;
}
...
function CircleButton({ children, onClick, disabled, title, active, 'aria-label': ariaLabel }: {
  ...
  return (
    <button onClick={onClick} disabled={disabled} title={title} aria-label={ariaLabel} style={{
      ...
      background: active ? 'rgba(234,88,12,0.18)' : 'var(--glass-bg-light)',
      border: active ? '1px solid rgba(234,88,12,0.30)' : '1px solid var(--glass-bg-light)',
      color: active ? 'rgba(234,88,12,0.9)' : disabled ? 'var(--glass-text-light)' : 'var(--text-muted)',
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '4px 0' }}>
      <div style={{
        ...
        background: 'rgba(234,88,12,0.18)', border: '1px solid rgba(234,88,12,0.22)',
...
function AgentText({ text }: { text: string }) {
  // Parse Manus-style reasoning from response
  const lines = text.split('\n');
  const reasoningLines: string[] = [];
  let mainResponse = text;
  let knowledgeRecalledCount = 0;
  const knowledgeItems: string[] = [];

  // Extract reasoning section if it looks structured
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    // Detect numbered steps (Manus style)
    if (/^\d+\.\s+/.test(line)) {
      reasoningLines.push(line);
      knowledgeRecalledCount++;
    }
    // Detect "Knowledge recalled" mentions
    if (line.toLowerCase().includes('knowledge') || line.toLowerCase().includes('recall')) {
      knowledgeItems.push(line);
    }
  }

  // If we found reasoning, extract main response after it
  if (reasoningLines.length > 0) {
    const reasoningText = reasoningLines.join('\n');
    mainResponse = text.replace(reasoningText, '').trim();
  }

  return (
    <div style={{ padding: '4px 0' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
        <img src="/neuro-icon-new.png" alt="" style={{ width: 18, height: 18, minWidth: 18, minHeight: 18, borderRadius: '50%', opacity: 0.75, flexShrink: 0 }} />
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>neuro</span>
        <span style={{ fontSize: 10, color: 'var(--text-ghost)' }}>
          {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
        </span>
      </div>

      {/* Manus-style reasoning breakdown if detected */}
            ...
            if (match) {
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
                  <span style={{ minWidth: 16, color: 'rgba(52,211,153,0.60)', fontWeight: 600, fontSize: 12 }}>✓</span>
                  <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{match[1]}</span>
                </div>
              );
            }
            return null;
      ...
      {knowledgeItems.length > 0 && (
        <div style={{ paddingLeft: 24, marginBottom: 6 }}>
          <button style={{
            ...
            color: 'var(--text-muted)', fontSize: 12, cursor: 'default',
          }}>
            <span>{'\u26A1'}</span>
            <span>Knowledge recalled({knowledgeItems.length})</span>
          </button>
        </div>
      )}
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isSelected ? 'rgba(59,130,246,0.20)' : 'var(--glass-bg-light)',
                border: isSelected ? '1px solid rgba(59,130,246,0.40)' : '1px solid var(--glass-bg-light)',
                color: isDimmed ? 'var(--text-ghost)' : isSelected ? 'rgba(59,130,246,0.95)' : 'var(--text-secondary)',
              ...
              onMouseLeave={(e) => { if (!isAnswered) e.currentTarget.style.background = 'var(--glass-bg-light)'; }}
            >
              <span>{i + 1}. {opt}</span>
              {isSelected && (
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
            <input
              ...
              style={{
                ...
                background: 'var(--glass-bg-light)', border: '1px solid var(--glass-bg-light)',
...
/** Icon picker for Manus-style tool pills — enhanced with harness status icons */
function toolPillIcon(label: string): string {
  const l = label.toLowerCase();
  // Web & Search Tools
  if (l.includes('search') || l.includes('webfetch') || l.includes('fetching') || l.includes('searxng')) return '🔍';
  // File & Content Reading
  if (l.includes('read') || l.includes('brows') || l.includes('page') || l.includes('scraping')) return '📖';
  // Writing & Creation
  if (l.includes('writ') || l.includes('creat') || l.includes('generat') || l.includes('compos') || l.includes('synthes')) return '✏️';
  // Computer & UI Control
  if (l.includes('computer') || l.includes('browser') || l.includes('using') || l.includes('control')) return '💻';
  // Memory & Knowledge
  if (l.includes('knowledge') || l.includes('memory') || l.includes('recall') || l.includes('store')) return '⚡';
  // Analysis & Thinking
  if (l.includes('think') || l.includes('routing') || l.includes('classif') || l.includes('analyz')) return '🧠';
  // Summarization
  if (l.includes('summariz')) return '📝';
  // Research
  if (l.includes('research')) return '🔬';
  // Execution & Scripting
  if (l.includes('code') || l.includes('run') || l.includes('exec') || l.includes('script')) return '</>';
  // Permission & Security
  if (l.includes('permission') || l.includes('confirm') || l.includes('approve')) return '🔐';
  // Progress & Monitoring
  if (l.includes('progress') || l.includes('track') || l.includes('monitor')) return '⏳';
  // Default
  return '⚙️';
}

/** Manus-style tool action pill -- gray rounded badge with icon and status indicator */
function SubStepPill({ label, status }: { label: string; status: 'pending' | 'running' | 'done' | 'error' | 'failed' }) {
  ...
  let bgColor = 'var(--glass-bg-light)';
  let textColor = 'var(--text-muted)';
  let indicatorColor = 'rgba(156, 163, 175, 0.5)';
  ...
  return (
    <div style={{
      ...
      marginBottom: 6,
      border: `1px solid ${indicatorColor}20`,
      ...
      {isActive && (
        <motion.span
          style={{ width: 6, height: 6, background: indicatorColor, flexShrink: 0, marginLeft: 2, display: 'inline-block' }}
          animate={{ borderRadius: ['15%', '50%', '15%'], scale: [1, 1.2, 1] }}
          transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
        />
      )}
      {isDone && (
        <span style={{
          width: 6, height: 6, background: indicatorColor, flexShrink: 0, marginLeft: 2,
...
/** Manus-style status indicator -- morphing square + label + permission/progress status */
interface ToolIndicatorProps {
  label: string;
  status?: 'idle' | 'running' | 'pending-permission' | 'blocked' | 'done';
  progressPercent?: number;
}

function ToolIndicator({ label, status = 'idle', progressPercent = 0 }: ToolIndicatorProps) {
  ...
  return (
    <div style={{
      ...
      {status === 'running' ? (
        <motion.div
          style={{ width: 8, height: 8, background: statusColor, flexShrink: 0 }}
          animate={{ borderRadius: ['15%', '50%', '15%'], scale: [1, 1.3, 1] }}
          transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
        />
      ) : (
        <div style={{ width: 8, height: 8, background: statusColor, flexShrink: 0, borderRadius: '50%' }} />
      )}
    ...
    <div style={{ marginBottom: 6 }}>
      ...
      {expanded && items && items.length > 0 && (
        <div style={{ paddingLeft: 16, paddingTop: 4 }}>
          {items.map((item, i) => (
            <div key={i} style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5, marginBottom: 2 }}>
              {item}
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ margin: '4px 0' }}>
      ...
      <div onClick={() => setExpanded(e => !e)} style={{
        ...
        {/* Task description */}
        <span style={{ flex: 1, fontSize: 14, color: 'var(--text-secondary)', fontWeight: 500 }}>
          {taskDescription}
        </span>
        {/* Chevron */}
                    ...
                    {isAgent && step.actionLog && step.actionLog.length > 0 && (
                      <div style={{ paddingLeft: 8, marginTop: 2, marginBottom: 4 }}>
                        {step.actionLog.slice(-5).map((entry, i) => (
                          <div key={`${entry.ts}_${i}`} style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            <span style={{ color: 'var(--text-ghost)', marginRight: 4 }}>{'\u2192'}</span>{entry.desc}
                          </div>
                        ))}
                      </div>
                    )}
                    ...
                    {isAgent && step.latestScreenshot && (
                      <div style={{ marginTop: 4, marginBottom: 4 }}>
                        <img src={`data:image/jpeg;base64,${step.latestScreenshot}`} alt="Agent view" style={{
                          ...
                          border: '1px solid var(--border-subtle)', display: 'block', cursor: 'pointer',
              ...
              {isComputerTask && latestScreenshot && !visibleSteps.some(s => s.latestScreenshot) && (
                <div style={{ marginTop: 4 }}>
                  <img src={`data:image/jpeg;base64,${latestScreenshot}`} alt="Computer view" onClick={onExpandComputer} style={{
                    width: 100, height: 62, objectFit: 'cover', borderRadius: 6,
                    border: '1px solid var(--border-subtle)', display: 'block', cursor: onExpandComputer ? 'pointer' : 'default',
                  }} />
                </div>
              )}
...
const StepRow = memo(function StepRow({ step }: { step: AgentStep }) {
  ...
  if (step.clarificationQuestion) {
    return (
      <div style={{ marginBottom: 6, padding: '8px 10px', borderRadius: 7, border: '1px solid rgba(59,130,246,0.35)', background: 'rgba(59,130,246,0.06)' }}>
        ...
        {step.status === 'done' && step.output && <div style={{ marginTop: 6, fontSize: 10, color: 'var(--text-muted)', paddingLeft: 16, lineHeight: 1.5 }}>{step.output}</div>}
  ...
  const statusColor = step.status === 'running' ? 'var(--text-primary)' : step.status === 'done' ? (step.timedOut ? 'rgba(251,191,36,0.65)' : 'var(--text-muted)') : step.status === 'error' ? 'rgba(239,68,68,0.80)' : 'var(--text-ghost)';
      ...
      {hasScreenshot && (
        <div style={{ marginLeft: 12, marginTop: 4 }}>
          ...
          <AnimatePresence>
            {showScreenshot && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.15 }} style={{ overflow: 'hidden' }}>
                <img src={`data:image/jpeg;base64,${step.latestScreenshot}`} alt="Agent view" style={{ marginTop: 4, marginLeft: 12, width: 80, height: 50, objectFit: 'cover', borderRadius: 5, border: '1px solid var(--border-subtle)', display: 'block', cursor: 'pointer' }}
            ...
            <div style={{ marginLeft: 12, marginTop: 2 }}>
              {(step.actionLog ?? []).map((entry, i) => (
                <div key={`${entry.ts}_${i}`} style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: entry.type === 'verify' ? (entry.desc.includes('OK') ? 'rgba(52,211,153,0.70)' : 'rgba(251,191,36,0.60)') : 'rgba(156,163,175,0.55)', lineHeight: 1.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  <span style={{ color: 'var(--text-ghost)', marginRight: 3 }}>-&gt;</span>{entry.desc}
                </div>
              ))}
            </div>
          )}
      ...
      <AnimatePresence>
        {showOutput && hasOutput && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.12 }} style={{ overflow: 'hidden' }}>
            <div style={{ marginTop: 4, marginLeft: 12, padding: '6px 8px', borderRadius: 6, background: 'var(--glass-bg-light)', border: '1px solid var(--glass-bg-light)', fontSize: 10, color: 'var(--text-muted)', lineHeight: 1.55, fontFamily: "'JetBrains Mono', monospace", whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
        ...
        <div style={{ marginLeft: 12, marginTop: 4 }}>
          {step.subSteps.map((ss, i) => (
            <div key={`${ss.label}_${i}`} style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
              <div style={{ width: 5, height: 5, borderRadius: '50%', background: ss.done ? 'rgba(52,211,153,0.60)' : 'var(--glass-text-light)', flexShrink: 0 }} />
              <span style={{ fontSize: 9, color: 'var(--text-muted)' }}>{ss.label}</span>
            </div>
          ))}
        </div>
      )}
...
function _RunBlock({ run, onDocClick, isLatest }: { run: AgentRun; onDocClick: (doc: AgentDocument) => void; isLatest?: boolean }) {
  ...
  return (
    <div style={{ background: 'var(--glass-bg-light)', border: '1px solid var(--glass-bg-light)', borderRadius: 8, marginBottom: 4, overflow: 'hidden' }}>
      {/* Header */}
      <button onClick={() => setCollapsed(c => !c)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
        ...
        <span style={{ flex: 1, fontSize: 11, color: 'var(--text-secondary)', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {run.userMessage.length > 50 ? run.userMessage.slice(0, 50) + '...' : run.userMessage}
        </span>
        <span style={{ fontSize: 10, color: 'var(--text-ghost)', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>{doneSteps}/{run.steps.length}</span>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ color: 'var(--text-ghost)', transform: collapsed ? 'rotate(0deg)' : 'rotate(180deg)', transition: 'transform 0.15s ease', flexShrink: 0 }}>
        ...
        {!collapsed && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.15 }} style={{ overflow: 'hidden' }}>
            <div style={{ padding: '0 10px 8px' }}>
                  ...
                  return (
                    <div key={step.id}>
                      <button onClick={() => setCollapsedSteps(prev => { const n = new Set(prev); if (n.has(step.id)) n.delete(step.id); else n.add(step.id); return n; })} style={{ display: 'flex', alignItems: 'center', gap: 5, width: '100%', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 0', textAlign: 'left' }}>
                        ...
                        <StepDot status={step.status} timedOut={step.timedOut} />
                        <span style={{ flex: 1, fontSize: 10, color: step.timedOut ? 'rgba(251,191,36,0.50)' : 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{step.label}</span>
                        {step.actionLog && step.actionLog.length > 0 && <span style={{ fontSize: 9, color: 'var(--text-ghost)', flexShrink: 0 }}>{step.actionLog.length} actions</span>}
                      </button>
                      {!isCollapsed && <StepRow step={step} />}
                    </div>
                  );
                }
                return <div key={step.id}><StepRow step={step} /></div>;
              })}
              ...
              {/* Done footer */}
              {run.status === 'done' && (
                <div style={{ marginTop: 10, paddingTop: 8, borderTop: '1px solid var(--glass-bg-light)' }}>
                  {run.finalAnswer ? (
                    <div style={{ background: 'rgba(52,211,153,0.06)', border: '1px solid rgba(52,211,153,0.15)', borderRadius: 8, padding: '8px 10px', marginBottom: 4 }}>
                      ...
                      <div style={{ fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: 'var(--text-primary)', lineHeight: 1.55, whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: 200, overflowY: 'auto' }}>{run.finalAnswer}</div>
                    </div>
                  ) : (
              ...
              {/* Error footer */}
              {run.status === 'error' && (
                <div style={{ marginTop: 8, paddingTop: 8, borderTop: '1px solid var(--glass-bg-light)' }}>
                  {run.errorDetail ? (
                    <div style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: 8, padding: '8px 10px' }}>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.45, whiteSpace: 'pre-wrap' }}>{run.errorDetail}</div>
                    </div>
...
export function ActionSidebarCompact({ machineId: _machineId, onComputerTask, agentState: _agentState, onExpandComputer }: ActionSidebarCompactProps) {
  ...
  const executeRun = useCallback(async (runId: string, userMessage: string) => {
    ...
    setRuns(prev => prev.map(r => r.id !== runId ? r : { ...r, steps: [{ id: thinkStepId, label: 'Classifying request', status: 'running', hidden: true }] }));
    scrollToBottom();
    ...
    try {
      ...
      // 3. If tool needed -> run tool FIRST -> call LLM with results -> loop if LLM wants more
        ...
        if (chatBudget.utilizationPct > 80) {
          ...
          history = compactMessages(history, targetTokens) as typeof history;
        ...
        if (toolClass === 'none') {
          ...
          responseText = sanitizeLLMOutput(chatResponse.replace(/^\[TOOL:[^\]]*\]\n?/, '').trim());
        }
        ...
        // ── Ask tool: show inline question, wait for answer, then respond ──
          ...
          responseText = sanitizeLLMOutput(clarifiedResponse.replace(/^\[TOOL:[^\]]*\]\n?/, '').trim());
        }
        ...
        // always has real data when composing the answer. No more "I can't tell you the weather".
          ...
          while (currentTool !== 'none' && currentTool !== 'ask' && iteration < MAX_TOOL_ITERATIONS) {
            ...
            switch (currentTool) {
              ...
              const isAbort = toolErr instanceof DOMException && toolErr.name === 'AbortError';
              if (isAbort) throw toolErr; // Re-throw aborts to be handled by outer catch
              result = `Sorry, I ran into an issue: ${toolErr instanceof Error ? toolErr.message : 'unknown error'}. Let me try a different approach.`;
              updateStep(runId, toolStepId, s => ({ ...s, status: 'error', output: result }));
            }
...
Based on this data, answer the user's question directly and conversationally. Be specific -- use actual numbers, names, and facts from the data. Use Celsius for temperatures, metric for distances. Be precise with data from the search results. Do not say you cannot access information -- you have it above. If the data does not contain what was asked, say so honestly.`;
            ...
            if (nextToolMatch) {
              ...
              const cleaned = sanitizeLLMOutput(llmResponse.replace(/\[TOOL:[^\]]+\]/g, '').trim());
              ...
              currentTool = 'none';
              responseText = sanitizeLLMOutput(llmResponse.replace(/^\[TOOL:[^\]]*\]\n?/, '').trim());
            }
          }
      ...
      // Mark run done
      updateRun(runId, r => {
        const answer = sanitizeLLMOutput(responseText.replace(/^Done:\s*/i, '').trim()) || undefined;
        const fallback = !answer ? sanitizeLLMOutput([...r.steps].reverse().find(s => s.output?.trim() && !s.hidden)?.output?.replace(/^Done:\s*/i, '').trim() || '') || undefined : undefined;
  ...
  for (const run of runs) {
    ...
    const thinkStep = run.steps.find(s => s.label === 'Classifying request' || s.label === 'Thinking');
  ...
  return (
    <>
      <div className="flex flex-col overflow-hidden" style={{
        ...
        borderRadius: 12, border: '1px solid var(--glass-bg-light)', boxShadow: '0 4px 24px rgba(0,0,0,0.4)',
      }}>
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
          {/* Header bar */}
          <div style={{
            display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px 7px',
            borderBottom: '1px solid var(--glass-bg-light)', flexShrink: 0,
          }}>
            <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
              {isRouting && (
                <motion.div
                  style={{ width: 6, height: 6, background: '#3b82f6', flexShrink: 0, display: 'inline-block' }}
                  animate={{ borderRadius: ['15%', '50%', '15%'], rotate: [0, 180, 360] }}
                  transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
                />
              )}
              <span style={{ fontSize: 12, color: 'var(--text-secondary)', fontWeight: 600 }}>neuro</span>
              {showThinkingDots && <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>classifying request</span>}
              {currentTotal > 0 && isRouting && (
                <span style={{ fontSize: 10, color: 'var(--text-ghost)', fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>{currentDone}/{currentTotal}</span>
              )}
              <span title={computerId} style={{ fontSize: 9, fontFamily: 'monospace', color: 'var(--text-ghost)', background: 'var(--glass-bg-light)', padding: '1px 5px', borderRadius: 4, flexShrink: 0, cursor: 'default', letterSpacing: '0.5px', marginLeft: 'auto' }}>{getSessionSuffix(computerId)}</span>
            </div>
              ...
              {chatEntries.length === 0 ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '40px 20px' }}>
                  <div style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-ghost)' }}>Ask neuro anything</div>
                  </div>
                </div>
              ) : (
                <AnimatePresence>
                  ...
                  {showThinkingDots && (
                    <div style={{ padding: '6px 0' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--text-muted)', fontSize: 13 }}>
                        <motion.div
                          style={{ width: 8, height: 8, background: '#3b82f6', flexShrink: 0 }}
                          animate={{ borderRadius: ['15%', '50%', '15%'], rotate: [0, 180, 360] }}
                          transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
                        />
                        <span>classifying request</span>
                      </div>
                    </div>
                  )}
                </AnimatePresence>
            ...
            <AnimatePresence>
              {showNewActivity && (
                <motion.button initial={{ opacity: 0, y: 4, scale: 0.92 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 4, scale: 0.92 }} transition={{ duration: 0.15 }} onClick={scrollToBottomNow} style={{
                  ...
                  background: 'rgba(234,88,12,0.20)', border: '1px solid rgba(234,88,12,0.35)',
                  color: 'rgba(234,88,12,0.90)', fontSize: 10, fontWeight: 600, cursor: 'pointer',
        ...
        <div style={{ padding: '10px 12px 12px', borderTop: 'none', flexShrink: 0 }}>
          <div style={{ position: 'relative' }}>
            <textarea ref={inputRef} value={input}
              ...
              style={{
                ...
                border: '1px solid var(--glass-bg-light)', outline: 'none', resize: 'none',
                minHeight: 40, maxHeight: 72, lineHeight: 1.5, fontFamily: 'inherit',
                backdropFilter: 'blur(8px)', transition: 'border-color 0.15s ease',
              }}
              onFocus={e => { e.currentTarget.style.borderColor = 'var(--glass-text-light)'; }}
              onBlur={e => { e.currentTarget.style.borderColor = 'var(--glass-bg-light)'; }}
            />
            ...
            <button
              ...
              style={{
                ...
                background: input.trim() ? 'rgba(234,88,12,0.25)' : 'var(--glass-bg-light)',
                border: input.trim() ? '1px solid rgba(234,88,12,0.40)' : '1px solid var(--glass-bg-light)',
                color: input.trim() ? 'rgba(234,88,12,0.90)' : 'var(--glass-text-light)',
          ...
          {isRouting && (
            <div style={{ marginTop: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              <span style={{ fontSize: 9, color: 'var(--text-ghost)' }}>neuro is processing - type to queue next task</span>
              <button onClick={() => abortRef.current?.abort()} style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                flexShrink: 0,
              }} title="Cancel current task">&#10005;</button>
            </div>
          )}
        ...
        {askUserRequest && !askUserRequest.isClarification && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.18 }} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              ...
              <button onClick={() => { askUserResolveRef.current?.({ value: 'abort', label: 'Abort' }); askUserResolveRef.current = null; setAskUserRequest(null); }} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-ghost)', fontSize: 12, padding: 0, lineHeight: 1 }} aria-label="Dismiss">x</button>
            </div>
            {askUserRequest.screenshot && <img src={`data:image/jpeg;base64,${askUserRequest.screenshot}`} alt="Current state" style={{ width: '100%', maxHeight: 120, objectFit: 'contain', borderRadius: 6, border: '1px solid var(--glass-bg-light)', marginBottom: 8, display: 'block' }} />}
            <p style={{ fontSize: 10, color: 'var(--text-secondary)', lineHeight: 1.5, margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{askUserRequest.question}</p>
            {askUserRequest.context && askUserRequest.context !== askUserRequest.question && <p style={{ fontSize: 9, color: 'var(--text-muted)', lineHeight: 1.4, margin: '5px 0 0', whiteSpace: 'pre-wrap' }}>{askUserRequest.context}</p>}
            <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
              {(askUserRequest.options?.length ? askUserRequest.options : [{ label: 'Proceed', value: 'proceed' }, { label: 'Skip', value: 'skip' }, { label: 'Abort', value: 'abort' }]).map(opt => (
                <button key={opt.value} onClick={() => { askUserResolveRef.current?.({ value: opt.value, label: opt.label }); askUserResolveRef.current = null; setAskUserRequest(null); }} style={{
                  ...
                  border: opt.value === 'proceed' ? '1px solid rgba(52,211,153,0.35)' : opt.value === 'abort' ? '1px solid rgba(239,68,68,0.30)' : '1px solid var(--border-subtle)',
                  background: opt.value === 'proceed' ? 'rgba(52,211,153,0.12)' : opt.value === 'abort' ? 'rgba(239,68,68,0.08)' : 'var(--glass-bg-light)',
                  color: opt.value === 'proceed' ? 'rgba(52,211,153,0.90)' : opt.value === 'abort' ? 'rgba(239,68,68,0.85)' : 'var(--text-muted)',
                }}>{opt.label}</button>
              ))}
            </div>
          </motion.div>
