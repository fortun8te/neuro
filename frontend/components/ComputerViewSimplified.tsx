...
function NeuroIcon({ size = 14 }: { size?: number }) {
  return (
    <img src="/neuro-icon-new.png" alt="" style={{ width: size, height: size, minWidth: size, minHeight: size, borderRadius: '50%', opacity: 0.75, flexShrink: 0 }} />
  );
}
...
function HealthDots({ health }: { health: InfrastructureHealth }) {
  ...
  const dot = (ok: boolean | null) => ok === null ? 'var(--text-muted)' : ok ? '#22c55e' : '#ef4444';
    ...
    <div style={{ position: 'relative', display: 'inline-flex' }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      ...
      {hovered && (
        <div style={{
          ...
          background: 'rgba(10,10,14,0.92)', backdropFilter: 'blur(12px)', border: '1px solid var(--border-subtle)',
          boxShadow: '0 4px 12px rgba(0,0,0,0.5)', whiteSpace: 'nowrap', fontSize: 10, fontWeight: 500,
          color: 'var(--text-primary)', zIndex: 100, pointerEvents: 'none',
        }}>
...
export function ComputerViewSimplified() {
  ...
  // Descriptive status: use the running step's instruction or the agent message, never generic "Working"/"Thinking"
  const runningStepLabel = runningIdx >= 0 ? agentState.steps[runningIdx].instruction : '';
  const currentStepDescription = isAgentActive
    ? (agentState.message && agentState.message !== 'Working...' && agentState.message !== 'Thinking'
        ? agentState.message
        : runningStepLabel || (agentState.phase === 'planning' ? 'classifying request' : agentState.phase === 'executing' ? 'executing step' : agentState.phase === 'verifying' ? 'verifying result' : 'processing'))
    : agentState.phase === 'done' ? 'Task completed'
    : agentState.phase === 'error' ? 'Task failed'
    : '';
  // Abort ref for cancel button
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#000', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 1, minWidth: 0 }}>
            ...
            {machines.length < MAX_MACHINES && (
              <button
                onClick={addMachine}
                title="Add computer"
                style={{ color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer', padding: 4, borderRadius: 6, transition: 'all 0.15s ease' }}
                onMouseEnter={e => { e.currentTarget.style.color = 'var(--text-secondary)'; e.currentTarget.style.background = 'var(--glass-border-light)'; }}
          ...
          {computerExpanded && (
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ fontSize: 10, color: 'var(--text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{activeMachine?.label}</span>
                {/* Collapse button */}
                <button
                  onClick={() => setComputerExpanded(false)}
                  title="Collapse computer view"
                  style={{
                    background: 'var(--glass-border-light)', border: '1px solid var(--border-subtle)',
                    borderRadius: 4, padding: '2px 6px', cursor: 'pointer',
                    color: 'var(--text-muted)', fontSize: 9, fontWeight: 600,
                    transition: 'all 0.15s ease',
                  }}
                  onMouseEnter={e => { e.currentTarget.style.color = 'var(--text-primary)'; e.currentTarget.style.background = 'var(--glass-bg-light)'; }}
                  onMouseLeave={e => { e.currentTarget.style.color = 'var(--text-muted)'; e.currentTarget.style.background = 'var(--glass-border-light)'; }}
                >
        ...
        {/* ── Bottom bar (36px) -- only visible when active or completed/error ── */}
        {(isAgentActive || agentState.phase === 'done' || agentState.phase === 'error') && (
        <div style={{
          height: 36, flexShrink: 0,
          display: 'flex', alignItems: 'center',
          padding: '0 16px',
          ...
          {/* Left: neuro icon + task details */}
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            {isAgentActive ? (
              <LoadingMorph size={10} color={agentState.phase === 'planning' ? '#8b5cf6' : '#3b82f6'} />
            ) : (
              <NeuroIcon size={14} />
            )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
            ...
            {isAgentActive && (
              <button onClick={() => { abortRef.current?.abort(); desktopBus.emit({ type: 'agent_status', phase: 'error', message: 'Cancelled by user' }); }} style={{
                width: 24, height: 24, borderRadius: '50%',
                background: 'var(--glass-border-light)', border: 'none',
                color: 'var(--text-secondary)', fontSize: 12, cursor: 'pointer',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }} title="Cancel task">&#10005;</button>
            )}
            <button
              onClick={() => setHumanControl(prev => !prev)}
              style={{
                fontSize: 9, fontWeight: 600, padding: '2px 8px', borderRadius: 4, cursor: 'pointer',
                background: humanControl ? 'rgba(251,191,36,0.12)' : 'var(--glass-border-light)',
                color: humanControl ? 'rgba(251,191,36,0.8)' : 'var(--text-muted)',
                border: `1px solid ${humanControl ? 'rgba(251,191,36,0.2)' : 'var(--glass-border-light)'}`,
