...
import { NeuroIcon } from './NeuroIcon';
...
export function MakeStudio() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .nomad-bar-shimmer {
        background: linear-gradient(90deg, transparent 0%, var(--text-ghost) 50%, transparent 100%);
        width: 40%;
        position: absolute;
        top: 0;
        bottom: 0;
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .cta button { width:100%; padding:${Math.round(htmlDim.h * 0.018)}px; font-size:${Math.round(htmlDim.w * 0.04)}px; font-weight:700; border-radius:12px; border:none; cursor:pointer; background:${theme === 'dark' ? '#3b82f6' : '#1e40af'}; color:#fff; }
...
6. CTA button: brand accent background, white text, full width. Always visible at bottom.
...
- Focus on INSTRUCTIONS: scene, colors (hex only), lighting, composition. Not descriptions.`
  ...
  const buildBrandVisualRules = useCallback(() => {
    ...
    rules.push('- Brand action color: use brand palette primary for ALL CTA buttons, badges, and interactive accent elements');
...
Output ONLY the complete HTML document. Start with <!DOCTYPE html>.`;
  ...
  // ── Render ──
  return (
    <div className={`h-full flex flex-col overflow-hidden ${theme === 'dark' ? 'bg-transparent' : 'bg-[#f7f7f8]'}`}>
        ...
        <div ref={galleryScrollRef} className="flex-1 h-full overflow-y-auto px-6 py-6 relative z-10 bg-transparent">
          ...
          {isGenerating && generatingForPrompt && (
            <div className={`sticky top-0 z-20 -mx-6 px-6 pt-2 pb-2 mb-2 bg-gradient-to-b ${theme === 'dark' ? 'from-zinc-900 via-zinc-900/95' : 'from-[#f7f7f8] via-[#f7f7f8]/95'} to-transparent`}>
              <div className={`rounded-xl border overflow-hidden ${theme === 'dark' ? 'bg-zinc-800/90 border-zinc-700/60' : 'bg-slate-50/95 border-slate-300/80 shadow-sm'}`}>
                    ...
                    <button
                      ...
                      className={`text-[10px] font-medium px-2.5 py-1 rounded-md transition-colors flex-shrink-0 ${
                        codeDrawerOpen
                          ...
                          : theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-700' : 'text-zinc-400 hover:text-slate-700 hover:bg-slate-200'
                      }`}
                    >
          ...
          {codeDrawerOpen && (isGenerating || llmOutput) && htmlEnabled && llmEnabled && (
            <div className={`mb-4 rounded-xl border overflow-hidden ${theme === 'dark' ? 'bg-zinc-800/60 border-zinc-700/50' : 'bg-slate-50 border-slate-300/80 shadow-sm'}`}>
              <div className="flex" style={{ height: '300px', maxHeight: '40vh' }}>
                ...
                {/* Right: Live preview */}
                <div className={`w-[280px] flex-shrink-0 border-l flex flex-col items-center justify-center p-3 ${theme === 'dark' ? 'border-zinc-700/50 bg-zinc-800/40' : 'border-slate-300 bg-slate-100/50'}`}>
                  {debouncedHtml ? (
                    <>
                      <div style={{
                        width: '240px',
                        ...
                        position: 'relative',
                        borderRadius: '8px',
                      }} className={`border ${theme === 'dark' ? 'border-zinc-700' : 'border-slate-400'}`}>
                        <iframe
          ...
          {activeMode === 'static' && htmlVariants.length > 0 && (
            <div className={`mb-6 rounded-2xl border p-4 ${theme === 'dark' ? 'bg-zinc-800/50 border-zinc-700/50' : 'bg-slate-50 border-slate-300/80 shadow-sm'}`}>
              {/* Action bar */}
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${theme === 'dark' ? 'bg-zinc-700 text-zinc-400' : 'bg-slate-200 text-slate-700'}`}>
                    {htmlVariants.length}
                  </span>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {selectedVariants.size < htmlVariants.length ? (
                    <button
                      ...
                      className={`text-[10px] px-2 py-0.5 rounded transition-colors ${theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-700' : 'text-zinc-400 hover:text-slate-700 hover:bg-slate-200'}`}
                    >
                      Select All
                    </button>
                  ) : (
                  ...
                  {selectedVariants.size > 0 && !isGenerating && !isRendering && (
                    <button
                      ...
                      className={`text-[10px] px-2.5 py-1 rounded-md font-semibold transition-colors bg-blue-600 text-white hover:bg-blue-500`}
                    >
                  ...
                  {!isGenerating && !isRendering && (
                    <button
                      ...
                      className={`text-[10px] px-2 py-0.5 rounded ${theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-700' : 'text-zinc-400 hover:text-slate-700 hover:bg-slate-200'}`}
                    >
                      Clear
                    </button>
                  )}
              ...
              {isRendering && (
                <div className={`mb-3 px-3 py-2 rounded-lg flex items-center gap-3 ${
                  theme === 'dark' ? 'bg-blue-900/20 border border-blue-800/40' : 'bg-blue-50 border border-blue-200'
                }`}>
                  <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className={`text-[10px] font-medium truncate ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'}`}>
                      {renderProgress || `Rendering ${renderCurrent}/${renderTotal}...`}
                    </div>
                  </div>
                  <div className={`w-20 h-1.5 rounded-full overflow-hidden flex-shrink-0 ${theme === 'dark' ? 'bg-blue-900/50' : 'bg-blue-200'}`}>
                    <div
                  ...
                  <button
                    ...
                    className={`text-[9px] px-2 py-0.5 rounded font-medium flex-shrink-0 ${theme === 'dark' ? 'text-blue-400 hover:bg-blue-800/50' : 'text-blue-600 hover:bg-blue-100'}`}
                  >
                    Cancel
                  </button>
                </div>
                    ...
                    <div key={variant.id} className="flex flex-col">
                      <div
                        className={`group relative rounded-xl overflow-hidden border-2 cursor-pointer transition-all hover:scale-[1.02] hover:shadow-lg ${
                          isSelected
                            ...
                            : theme === 'dark' ? 'border-zinc-700 hover:border-zinc-500' : 'border-slate-300 hover:border-zinc-400'
                        }`}
                      >
                        ...
                        {variant.inspiredBy && (
                          <span className={`absolute top-2 left-2 z-10 px-1.5 py-0.5 rounded text-[8px] font-medium backdrop-blur-sm ${
                            theme === 'dark' ? 'bg-blue-900/60 text-blue-300' : 'bg-blue-100/90 text-blue-700'
                          }`}>
                            {variant.inspiredBy}
                          </span>
                        )}
                          ...
                          <div className={`absolute bottom-0 left-0 right-0 px-2 py-1.5 flex items-end justify-between ${theme === 'dark' ? 'bg-gradient-to-t from-black/80 to-transparent' : 'bg-gradient-to-t from-black/60 to-transparent'}`}>
                            ...
                            {variant.renders.length > 0 && (
                              <button
                                ...
                                className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-blue-500/80 text-white hover:bg-blue-500 transition-colors flex-shrink-0 ml-1"
                              >
                          ...
                          {variant.visionFeedback && (
                            <div className="absolute top-2 right-2 z-10">
                              <span className={`px-1.5 py-0.5 rounded text-[8px] font-medium backdrop-blur-sm ${
                                theme === 'dark' ? 'bg-blue-900/70 text-blue-300' : 'bg-blue-100/90 text-blue-700'
                              }`}>
                                QA
                              </span>
                            </div>
                      ...
                      {variant.visionFeedback && isExpanded && (
                        <div className={`mt-1 rounded-lg border p-2.5 text-[11px] whitespace-pre-wrap leading-relaxed ${
                          theme === 'dark' ? 'bg-blue-900/10 border-blue-800/30 text-zinc-300' : 'bg-blue-50 border-blue-200 text-zinc-700'
                        }`}>
                          <p className={`text-[9px] uppercase tracking-wider font-semibold mb-1 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>Vision QA</p>
                          {variant.visionFeedback}
                        </div>
                      )}
                      {/* Expandable render strip */}
                      {isExpanded && variant.renders.length > 0 && (
                        <div className={`mt-1 rounded-lg border overflow-hidden ${theme === 'dark' ? 'bg-zinc-800/50 border-zinc-700/50' : 'bg-zinc-50 border-slate-300'}`}>
          ...
          {activeMode === 'static' && storedImages.length > 0 && (
            <div>
              ...
              <div className="flex items-center justify-between mb-4">
                    ...
                    <select
                      ...
                      className={`text-[10px] font-medium py-0.5 px-1 rounded border appearance-none cursor-pointer ${
                        theme === 'dark'
                          ...
                          : 'bg-white border-slate-300 text-zinc-500'
                      }`}
                    >
                      {GRID_OPTIONS.map(n => (
                ...
                <div className="flex items-center gap-1 flex-wrap">
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-full text-[10px] font-medium transition-colors ${!favoriteFilter && !pipelineFilter ? (theme === 'dark' ? 'bg-blue-600 text-white' : 'bg-blue-600 text-white') : (theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300' : 'text-zinc-400 hover:text-zinc-600')}`}
                  >All</button>
                  <button
                    onClick={() => { setFavoriteFilter(true); setPipelineFilter(null); }}
                    className={`px-2.5 py-1 rounded-full text-[10px] font-medium transition-colors flex items-center gap-1 ${favoriteFilter ? (theme === 'dark' ? 'bg-blue-600 text-white' : 'bg-blue-600 text-white') : (theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300' : 'text-zinc-400 hover:text-zinc-600')}`}
                  >Saved</button>
                  ...
                  <select
                    ...
                    className={`text-[9px] font-medium py-0.5 px-1.5 rounded border appearance-none cursor-pointer ${
                      theme === 'dark' ? 'bg-zinc-800 border-zinc-700 text-zinc-400' : 'bg-white border-slate-300 text-zinc-500'
                    }`}
                  >
              ...
              {showVisionComparison && visionHistory.length > 0 && (
                <div className={`mb-6 rounded-xl border overflow-hidden ${theme === 'dark' ? 'bg-zinc-900/80 border-blue-500/30' : 'bg-blue-50/30 border-blue-200'}`}>
                  {/* Header */}
                  <div className={`flex items-center justify-between px-4 py-2.5 border-b ${theme === 'dark' ? 'border-blue-500/20' : 'border-blue-200/60'}`}>
                    <div className="flex items-center gap-2.5">
                      <div className={`w-2 h-2 rounded-full ${isGenerating ? 'bg-blue-500 animate-pulse' : 'bg-emerald-500'}`} />
                      <span className={`text-xs font-semibold ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'}`}>
                        Vision QA
                      </span>
                      <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${theme === 'dark' ? 'bg-blue-500/15 text-blue-400' : 'bg-blue-100 text-blue-600'}`}>
                  ...
                  <div className="p-4">
                    ...
                    {visionHistory.length >= 2 && (
                      <div className="flex gap-4 mb-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className={`relative rounded-lg overflow-hidden border ${theme === 'dark' ? 'border-zinc-700' : 'border-slate-300'}`}>
                            <img
                        ...
                        <div className="flex items-center pt-5">
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className={theme === 'dark' ? 'text-blue-500' : 'text-blue-400'}>
                            <path d="M5 12h14M12 5l7 7-7 7" />
                          </svg>
                        </div>
                        {/* Latest */}
                        <div className="flex-1">
                          <p className={`text-[9px] uppercase tracking-widest font-bold mb-1.5 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>
                            Round {visionHistory[visionHistory.length - 1].round}
                          </p>
                          <div className={`relative rounded-lg overflow-hidden border-2 ${theme === 'dark' ? 'border-blue-500/50 ring-1 ring-blue-500/20' : 'border-blue-400 ring-1 ring-blue-400/20'}`}>
                            <img
                              ...
                              <div className={`relative rounded-lg overflow-hidden border ${
                                idx === visionHistory.length - 1
                                  ...
                                  : idx === 0
                                    ...
                                    : (theme === 'dark' ? 'border-zinc-700/50' : 'border-slate-300')
                              }`}>
                                <img
                    ...
                    {visionHistory.length === 1 && (
                      <div className="flex items-center gap-3">
                        <div className={`w-24 rounded-lg overflow-hidden border ${theme === 'dark' ? 'border-zinc-700' : 'border-slate-300'}`}>
                          <img
                            src={visionHistory[0].screenshot.startsWith('data:') ? visionHistory[0].screenshot : `data:image/png;base64,${visionHistory[0].screenshot}`}
                            alt="Reviewing..."
                            className="w-full object-contain"
                        ...
                        <div className="flex-1">
                          <p className={`text-xs font-medium ${theme === 'dark' ? 'text-blue-300' : 'text-blue-600'}`}>
                  ...
                  <div key={promptText} className="mb-6">
                    ...
                    <div className="flex items-center justify-between mb-2 px-0.5">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${theme === 'dark' ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-200 text-slate-700'}`}>{first.aspectRatio}</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${theme === 'dark' ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-200 text-slate-700'}`}>{modelDisplayName(first.model)}</span>
                    ...
                    <div className="grid gap-2 items-start" style={{ gridTemplateColumns: `repeat(${gridCols}, minmax(0, 1fr))` }}>
                      ...
                      {images.map((img) => {
                        ...
                        >
                          <div className={`${getAspectClass(img.aspectRatio)} overflow-hidden ${theme === 'dark' ? 'bg-zinc-900' : 'bg-slate-200'}`}>
                            ...
                            <div className="flex items-center gap-1">
                              {pipelineShort && (
                                <span className={`text-[7px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${
                                  img.pipeline === 'reference-copy' ? 'bg-blue-500/50 text-blue-100' :
          ...
          {activeMode === 'static' && !isGenerating && storedImages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center min-h-[400px]">
              <div className={`w-20 h-20 rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.06),0_1px_3px_rgba(0,0,0,0.04)] border border-dashed flex items-center justify-center ${theme === 'dark' ? 'bg-zinc-800 border-zinc-700' : 'bg-slate-50 border-slate-400'}`}>
                <NeuroIcon size={32} />
              </div>
              <div>
      ...
      {/* ── Bottom Bar ── */}
      <div className={`flex-shrink-0 px-6 py-4 ${theme === 'dark' ? 'border-t border-zinc-800' : 'border-t border-slate-300'}`}>
        {/* Mode Tabs */}
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all duration-200 ${
                activeMode === mode.key
                  ...
                  : theme === 'dark'
                    ...
                    : 'bg-slate-200 text-slate-700 hover:bg-slate-300 shadow-[0_1px_2px_rgba(0,0,0,0.04)] hover:shadow-[0_2px_4px_rgba(0,0,0,0.08)] hover:-translate-y-px'
              }`}
            >
        ...
        {activeMode !== 'funnel' && <div className="max-w-[960px] mx-auto">
                ...
                {referenceCopyTarget ? (
                  <>
                    <img
                      ...
                      alt="Reference"
                      className="w-12 h-12 rounded-lg object-cover border border-blue-400/30"
                    />
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs font-semibold truncate ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'}`}>
                        {referenceCopyTarget.filename}
                      </p>
                      <p className={`text-[10px] truncate ${theme === 'dark' ? 'text-blue-400/70' : 'text-blue-500'}`}>
                        {referenceCopyTarget.category} — {referenceCopyTarget.description.slice(0, 80)}...
                      </p>
                    </div>
                    <button
                      onClick={() => setShowAdLibrary(true)}
                      className={`text-[10px] px-2 py-1 rounded-md font-medium ${theme === 'dark' ? 'text-blue-300 hover:bg-blue-500/20' : 'text-blue-600 hover:bg-blue-100'}`}
                    >
                      Change
                    </button>
                    <button
                  ...
                  <button
                    ...
                    className={`flex-1 text-xs font-medium py-2 text-center rounded-lg border border-dashed transition-colors ${
                      theme === 'dark' ? 'text-blue-400 border-blue-500/40 hover:bg-blue-500/10' : 'text-blue-600 border-blue-300 hover:bg-blue-50'
                    }`}
                  >
              ...
              {referenceCopyTarget && (
                <div className={`px-3 pb-2.5 border-t ${theme === 'dark' ? 'border-blue-500/20' : 'border-blue-200/60'}`}>
                  <div className="flex items-center justify-between mt-2 mb-1">
                    ...
                    {referenceStyle && (
                      <span className={`text-[8px] px-1.5 py-0.5 rounded-full font-medium ${theme === 'dark' ? 'bg-blue-500/20 text-blue-300' : 'bg-blue-100 text-blue-600'}`}>
          ...
          <div className={`rounded-2xl border overflow-hidden transition-shadow duration-200 ${theme === 'dark' ? 'bg-zinc-800/60 border-zinc-700/60 shadow-[0_1px_3px_rgba(0,0,0,0.15),0_2px_8px_rgba(0,0,0,0.08)] focus-within:shadow-[0_2px_6px_rgba(0,0,0,0.2),0_4px_12px_rgba(0,0,0,0.12)]' : 'bg-zinc-50 border-slate-300/80 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_2px_8px_rgba(0,0,0,0.03)] focus-within:shadow-[0_2px_6px_rgba(0,0,0,0.06),0_4px_12px_rgba(0,0,0,0.04)]'}`} onDragOver={handleImageDragOver} onDrop={handleImageDrop}>
            ...
            {/* Template badge */}
            {templateHtml && (
              <div className={`flex items-center gap-2 px-4 pb-2 ${uploadedImages.length === 0 ? 'border-t pt-2' : ''} ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-200'}`}>
                <span className={`inline-flex items-center gap-1.5 pl-2 pr-1 py-1 rounded-full text-xs font-medium border ${theme === 'dark' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                  ...
                  <button
                    ...
                    className={`ml-0.5 p-0.5 rounded-full transition-colors ${theme === 'dark' ? 'hover:bg-blue-500/20' : 'hover:bg-blue-100'}`}
                  >
            ...
            {uploadedImages.length > 0 && (
              <div className={`flex flex-wrap gap-2 px-4 pb-3 border-t pt-2 ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-200'}`}>
                {uploadedImages.map((img, idx) => {
                  const chipColors: Record<string, string> = theme === 'dark' ? {
                    product: 'bg-blue-900/30 text-blue-300 border-blue-800/50',
                    layout: 'bg-blue-900/30 text-blue-300 border-blue-800/50',
                  } : {
            ...
            <div className={`flex items-center justify-between px-5 py-2.5 border-t ${theme === 'dark' ? 'border-zinc-800 bg-zinc-800/30' : 'border-slate-200 bg-zinc-50'}`}>
              <div className="flex items-center gap-2 min-w-0 overflow-x-auto">
                {/* Pipeline label */}
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium whitespace-nowrap ${theme === 'dark' ? 'bg-zinc-700 text-zinc-300' : 'bg-slate-200 text-slate-700'}`}>
                  {htmlEnabled && llmEnabled ? (
                  ...
                  <>
                    <button
                      ...
                      className={`px-2 py-0.5 rounded-full text-[9px] font-medium transition-colors whitespace-nowrap ${
                        theme === 'dark' ? 'bg-zinc-700/50 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-700' : 'bg-slate-200 text-slate-600 hover:text-slate-800 hover:bg-slate-300'
                      }`}
                ...
                {researchEnabled && copyComplete && (
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium whitespace-nowrap ${theme === 'dark' ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-700'}`}>
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium transition-colors whitespace-nowrap ${
                    adLibraryEnabled
                      ? theme === 'dark' ? 'bg-blue-900/40 text-blue-300 border border-blue-500/30' : 'bg-blue-50 text-blue-600 border border-blue-200'
                ...
                >
                  <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${adLibraryEnabled ? 'bg-blue-500' : theme === 'dark' ? 'bg-zinc-600' : 'bg-zinc-300'}`} />
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-all relative ${theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800' : 'text-zinc-400 hover:text-slate-800 hover:bg-slate-200'}`}
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-all ${theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800' : 'text-zinc-400 hover:text-slate-800 hover:bg-slate-200'}`}
                >
                ...
                {showSettings &&
                  createPortal(
                    <div
                      ref={popoverRef}
                      className={`fixed w-[280px] rounded-xl z-[9999] flex flex-col max-h-[75vh] overflow-hidden ${theme === 'dark' ? 'bg-zinc-900 shadow-[0_8px_30px_rgba(0,0,0,0.5),0_0_0_1px_rgba(0,0,0,0.6)]' : 'bg-slate-50 shadow-[0_8px_30px_rgba(0,0,0,0.12),0_0_0_1px_rgba(100,116,139,0.2)]'}`}
                      style={{
                    ...
                    >
                      <div className="overflow-y-auto px-4 py-3 space-y-3">
                        ...
                        <div>
                          ...
                          {/* Image model */}
                          <div className={`flex rounded-md overflow-hidden border ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-300'}`}>
                                  ...
                                  <button key={csIdx}
                                    ...
                                    className={`group relative px-2 py-0.5 rounded text-[10px] font-medium transition-all border ${
                                      customStyleImage === cs.base64
                                        ...
                                        : theme === 'dark' ? 'border-zinc-700 text-zinc-500 hover:text-zinc-300 hover:border-zinc-600' : 'border-slate-300 text-zinc-400 hover:text-zinc-600'
                                    }`}
                                  >
                                    {cs.name}
                                    <span
                            ...
                            <div className="mt-2">
                              ...
                              {customStyleImage && (
                                <div className="mt-1.5 flex items-center gap-2">
                                  <img src={customStyleImage} alt="Style ref" className="w-8 h-8 rounded object-cover border border-blue-400/30" />
                                  <span className={`text-[10px] ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>{customStyleName}</span>
                                </div>
                              )}
                            </div>
                          </div>
                        ...
                        {/* ── PIPELINE ── */}
                        <div className={`border-t pt-3 ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-200'}`}>
                          ...
                          {llmEnabled && (
                            <>
                                ...
                                <button onClick={() => setPresetEnabled(!presetEnabled)}
                                  className={`px-2 py-0.5 rounded text-[9px] font-medium transition-all border ${
                                    presetEnabled
                                      ...
                                      : theme === 'dark' ? 'border-zinc-700 text-zinc-600' : 'border-slate-300 text-zinc-400'
                                  }`}
                              ...
                              <div className={`flex rounded-md overflow-hidden border mb-2 ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-300'}`}>
                                ...
                                <button onClick={() => { setHtmlEnabled(true); setReferenceCopyEnabled(false); }}
                                  className={`flex-1 py-1.5 text-[10px] font-medium transition-all border-x ${
                                    htmlEnabled ? 'bg-blue-500 text-white border-blue-500' : (theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300 border-zinc-800' : 'text-zinc-400 hover:text-zinc-600 border-slate-300')
                                  }`}
                                >HTML ad</button>
                                <button onClick={() => { const next = !referenceCopyEnabled; setReferenceCopyEnabled(next); localStorage.setItem('make_reference_copy', String(next)); if (next) setHtmlEnabled(false); }}
                                  className={`flex-1 py-1.5 text-[10px] font-medium transition-all ${
                                    referenceCopyEnabled ? 'bg-blue-500 text-white' : theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300' : 'text-zinc-400 hover:text-zinc-600'
                                  }`}
                                >Clone ref</button>
                              </div>
                            </>
                          ...
                          {/* HTML sub-options */}
                          {htmlEnabled && llmEnabled && (
                            <div className={`space-y-1.5 mb-2 pl-2 border-l-2 ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-300'}`}>
                              <div className="flex items-center justify-between">
                                ...
                                <input type="number" min="1" max="10" value={variantCount}
                                  ...
                                  className={`w-12 text-[10px] font-medium rounded px-1.5 py-0.5 text-center focus:outline-none ${theme === 'dark' ? 'text-zinc-200 bg-zinc-800 border border-zinc-700' : 'text-zinc-800 bg-white border border-slate-300'}`}
                                />
                              </div>
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-0.5">
                                  {[1, 2, 5].map(n => (
                                    <button key={n} onClick={() => setRenderCount(n)}
                                      className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${renderCount === n ? 'bg-blue-500 text-white' : theme === 'dark' ? 'bg-zinc-800 text-zinc-500' : 'bg-slate-200 text-slate-700'}`}
                                    >{n}</button>
                                  ))}
                                </div>
                              </div>
                          ...
                          {/* Clone ref batch */}
                          {referenceCopyEnabled && (
                            <div className={`flex items-center justify-between mb-2 pl-2 border-l-2 ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-300'}`}>
                              ...
                              <div className="flex gap-0.5">
                                {[1, 3, 5].map(n => (
                                  <button key={n} onClick={() => { setBatchRefCount(n); localStorage.setItem('make_batch_ref_count', String(n)); }}
                                    className={`text-[9px] px-1.5 py-0.5 rounded font-medium ${batchRefCount === n ? 'bg-blue-500 text-white' : theme === 'dark' ? 'bg-zinc-800 text-zinc-500' : 'bg-slate-200 text-slate-700'}`}
                                  >{n}</button>
                                ))}
                              </div>
                            </div>
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    {visionFeedbackEnabled && (
                                      <input
                                        ...
                                        className={`w-10 text-center text-[9px] font-medium rounded px-1 py-0.5 focus:outline-none ${theme === 'dark' ? 'text-blue-300 bg-zinc-800 border border-zinc-700' : 'text-blue-700 bg-white border border-slate-300'}`}
                                      />
                                    )}
                                    <button onClick={() => { const next = !visionFeedbackEnabled; setVisionFeedbackEnabled(next); localStorage.setItem('make_vision_feedback', String(next)); }}
                                      className={`relative w-7 h-[16px] rounded-full transition-colors ${visionFeedbackEnabled ? 'bg-blue-500' : theme === 'dark' ? 'bg-zinc-700' : 'bg-zinc-300'}`}
                                    >
                        ...
                        {researchEnabled && hasResearchData && (
                          <div className={`border-t pt-2 ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-200'}`}>
                      ...
                      {/* Footer */}
                      <div className={`px-4 py-2 border-t ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-200'}`}>
                      ...
                      {htmlEnabled && llmEnabled ? (
                        <span className="text-xs font-semibold flex items-center gap-0.5">
                          <input
                            ...
                            className="w-8 bg-transparent text-center text-xs font-semibold [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none focus:outline-none rounded"
                            style={{
                              color: theme === 'dark' ? 'inherit' : 'inherit',
                              '--tw-ring-color': theme === 'dark' ? 'rgba(255,255,255,0.2)' : 'rgba(100,116,139,0.1)',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" onClick={() => setSelectedImage(null)}>
          ...
          <div
            className={`relative z-10 rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex ${
              theme === 'dark'
                ? 'bg-zinc-900 shadow-[0_8px_30px_rgba(0,0,0,0.5),0_4px_12px_rgba(0,0,0,0.3),0_0_0_1px_var(--glass-border-light)]'
                : 'bg-white shadow-[0_8px_30px_rgba(0,0,0,0.12),0_4px_12px_rgba(0,0,0,0.08),0_0_0_1px_rgba(0,0,0,0.04)]'
            }`}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Image */}
            <div className={`flex-1 flex items-center justify-center p-6 min-w-0 ${theme === 'dark' ? 'bg-zinc-950' : 'bg-slate-100'}`}>
              <img
            ...
            {/* Info sidebar */}
            <div className={`w-72 border-l p-5 overflow-y-auto flex-shrink-0 ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-300'}`}>
              <div className="flex items-center justify-between mb-4">
                <h3 className={`text-sm font-bold ${theme === 'dark' ? 'text-zinc-100' : 'text-zinc-900'}`}>{selectedImage.label}</h3>
                <button
                  onClick={() => setSelectedImage(null)}
                  className={`p-1.5 rounded-lg transition-colors ${theme === 'dark' ? 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800' : 'text-zinc-400 hover:text-slate-700 hover:bg-slate-200'}`}
                >
              ...
              <div className="space-y-2.5">
                <div className="grid grid-cols-2 gap-2">
                  <div className={`rounded-lg p-2.5 ${theme === 'dark' ? 'bg-zinc-800/80' : 'bg-slate-100'}`}>
                      ...
                      <div className="flex gap-1.5 mt-1 flex-wrap">
                        {selectedImage.referenceImages.map((refImg, i) => (
                          <img
                            ...
                            alt={`@img${i + 1}`}
                            className={`w-10 h-10 rounded-md object-cover border ${theme === 'dark' ? 'border-zinc-700' : 'border-slate-300'}`}
                            title={`@img${i + 1}`}
                          />
                        ))}
                      </div>
                ...
                {selectedImage.htmlScreenshot && (
                  <div className={`rounded-lg p-2.5 border ${theme === 'dark' ? 'bg-blue-900/20 border-blue-800/40' : 'bg-blue-50 border-blue-100'}`}>
                    <p className={`text-[9px] uppercase tracking-wider font-semibold mb-1.5 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-400'}`}>Layout Wireframe</p>
                    <img
                      src={`data:image/png;base64,${selectedImage.htmlScreenshot}`}
                      alt="HTML Layout"
                      className={`w-full rounded-md border shadow-sm ${theme === 'dark' ? 'border-blue-800/40' : 'border-blue-200'}`}
                    />
                  </div>
                )}
                ...
                {selectedImage.visionRounds && selectedImage.visionRounds.length > 0 ? (
                  <div className={`rounded-lg border overflow-hidden ${theme === 'dark' ? 'bg-blue-900/10 border-blue-800/40' : 'bg-blue-50/50 border-blue-200'}`}>
                    {/* Header */}
                    <div className={`px-2.5 py-2 flex items-center justify-between ${theme === 'dark' ? 'bg-blue-900/20' : 'bg-blue-100/50'}`}>
                      <p className={`text-[9px] uppercase tracking-wider font-semibold ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>
                      ...
                      {selectedImage.visionRounds.length > 1 && (
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className={`w-5 h-5 flex items-center justify-center rounded text-[10px] transition-colors disabled:opacity-30 ${theme === 'dark' ? 'text-blue-300 hover:bg-blue-800/40' : 'text-blue-600 hover:bg-blue-200'}`}
                          >
                          ...
                          <span className={`text-[9px] font-mono font-semibold min-w-[2rem] text-center ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'}`}>
                    ...
                    {selectedImage.visionRounds.length > 1 && (
                      <div className={`px-2.5 py-1.5 flex gap-1 overflow-x-auto border-b ${theme === 'dark' ? 'border-blue-800/30' : 'border-blue-200/60'}`}>
                      ...
                      return (
                        <div className="px-2.5 py-2 space-y-1.5">
                          ...
                          {round.feedback && round.feedback !== 'Original' && (
                            <div>
                              <p className={`text-[9px] font-semibold mb-0.5 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-500'}`}>Feedback</p>
                          ...
                          {round.imageBase64 !== selectedImage.imageBase64 && (
                            <button
                              ...
                              className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-700'}`}
                            >
                              View this version
                            </button>
                          )}
                  ...
                  <div className={`rounded-lg p-2.5 border ${theme === 'dark' ? 'bg-blue-900/20 border-blue-800/40' : 'bg-blue-50 border-blue-100'}`}>
                    <p className={`text-[9px] uppercase tracking-wider font-semibold mb-1 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>Vision QA</p>
                    <p className={`text-[11px] leading-relaxed ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'}`}>{selectedImage.visionFeedback}</p>
                  </div>
                ) : null}
                ...
                {selectedImage.strategyLabel && (
                  <div className={`rounded-lg px-2.5 py-2 border ${theme === 'dark' ? 'bg-blue-900/20 border-blue-800/40' : 'bg-blue-50 border-blue-100'}`}>
                    <p className={`text-[9px] uppercase tracking-wider font-semibold mb-0.5 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-400'}`}>Strategy</p>
                    <p className={`text-[11px] font-medium ${theme === 'dark' ? 'text-blue-300' : 'text-blue-700'}`}>{selectedImage.strategyLabel}</p>
                  </div>
                )}

                {/* HTML source (collapsible) */}
                {selectedImage.htmlSource && (
                  <details className={`rounded-lg p-2.5 border ${theme === 'dark' ? 'bg-zinc-800/80 border-zinc-700' : 'bg-zinc-50 border-slate-300'}`}>
                ...
                {selectedImage.htmlSource && (
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border transition-colors ${
                      theme === 'dark'
                        ? 'text-blue-300 bg-blue-900/30 hover:bg-blue-900/50 border-blue-800/40'
                ...
                {/* ── Refine Chat ── */}
                <div className={`pt-3 border-t ${theme === 'dark' ? 'border-zinc-700' : 'border-slate-300'}`}>
                    ...
                    <textarea
                      ...
                      className={`flex-1 text-[11px] px-2.5 py-2 rounded-lg border resize-none focus:outline-none disabled:opacity-50 ${
                        theme === 'dark'
                          ...
                          : 'bg-white border-slate-300 text-zinc-800 placeholder-zinc-400 focus:border-blue-300'
                      }`}
                    />
                    <button
                      ...
                      className={`self-end px-3 py-2 rounded-lg text-[10px] font-semibold transition-colors ${
                        isRefining
                          ...
                          : refinePrompt.trim()
                            ...
                            : theme === 'dark' ? 'bg-zinc-800 text-zinc-600 cursor-not-allowed' : 'bg-slate-200 text-slate-500 cursor-not-allowed'
                      }`}
                    >
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg transition-colors ${
                      theme === 'dark' ? 'text-zinc-300 bg-zinc-800 hover:bg-zinc-700' : 'text-slate-700 bg-slate-200 hover:bg-slate-300'
                    }`}
                  >
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-1 px-3 py-2 text-xs font-medium rounded-lg transition-colors ${
                      selectedImage.favorite
                        ...
                        : theme === 'dark' ? 'text-zinc-500 bg-zinc-800 hover:bg-zinc-700' : 'text-slate-600 bg-slate-200 hover:bg-slate-300'
                    }`}
                  >
          ...
          <div
            className={`relative z-10 w-full max-w-3xl max-h-[85vh] flex flex-col rounded-2xl overflow-hidden ${
              theme === 'dark'
                ...
                : 'bg-white border border-slate-300/80 shadow-[0_24px_80px_rgba(0,0,0,0.15)]'
            }`}
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className={`flex items-center justify-between px-6 py-4 border-b ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-300'}`}>
              <div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto">
              ...
              <div
                className={`p-5 border-b ${theme === 'dark' ? 'border-zinc-800' : 'border-slate-300'}`}
                onDrop={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  const files = Array.from(e.dataTransfer.files).filter(
              ...
              >
                  ...
                  <div className="space-y-3 mb-3">
                    {uploadedImages.map((img, idx) => (
                      <div key={idx} className={`flex gap-3 p-3 rounded-xl border ${
                        theme === 'dark' ? 'border-zinc-800 bg-zinc-800/30' : 'border-slate-300 bg-zinc-50'
                      }`}>
                        {/* Thumbnail */}
                        <div className="relative shrink-0">
                          <img
                            ...
                            className={`w-20 h-20 object-cover rounded-lg border ${
                              theme === 'dark' ? 'border-zinc-700' : 'border-slate-300'
                            }`}
                          />
                            ...
                            <input
                              ...
                              className={`flex-1 text-xs px-2 py-1 rounded-md border font-medium ${
                                theme === 'dark'
                                  ...
                                  : 'bg-white border-slate-300 text-zinc-800 placeholder-zinc-400'
                              }`}
                            />
                            <select
                              value={img.type}
                              ...
                              className={`text-[10px] px-1.5 py-1 rounded-md border ${
                                theme === 'dark'
                                  ...
                                  : 'bg-white border-slate-300 text-zinc-600'
                              }`}
                            >
                          ...
                          <textarea
                            ...
                            className={`w-full text-[11px] px-2 py-1.5 rounded-md border resize-none leading-relaxed ${
                              theme === 'dark'
                                ...
                                : 'bg-white border-slate-300 text-zinc-700 placeholder-zinc-400'
                            }`}
                          />
                        </div>
                      </div>
                ...
                <div
                  className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-colors ${
                    theme === 'dark' ? 'border-zinc-700 hover:border-zinc-500 text-zinc-500' : 'border-slate-300 hover:border-zinc-400 text-zinc-400'
                  }`}
              ...
              {campaign?.presetData && (() => {
                const preset = campaign.presetData!;
                const borderCls = theme === 'dark' ? 'border-zinc-800' : 'border-slate-300';
                const bgCls = theme === 'dark' ? 'bg-zinc-800/50' : 'bg-slate-100';
                const hoverCls = theme === 'dark' ? 'hover:bg-zinc-800/50' : 'hover:bg-slate-200/50';
              ...
              {!campaign?.presetData && campaign && (
                <div className="p-5">
                  <div className={`rounded-xl border p-4 space-y-2 text-xs font-mono ${
                    theme === 'dark' ? 'bg-zinc-800/50 border-zinc-700/50 text-zinc-400' : 'bg-zinc-50 border-slate-300 text-zinc-600'
                  }`}>
                ...
                {knowledgeDirty && (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${
                      knowledgeSaving
                        ? (theme === 'dark' ? 'bg-zinc-700 text-zinc-500' : 'bg-slate-200 text-slate-500')
                        : (theme === 'dark' ? 'bg-blue-600 text-white hover:bg-blue-500' : 'bg-blue-500 text-white hover:bg-blue-600')
                    }`}
                  >
              ...
              <textarea
                ...
                className={`flex-1 min-h-0 w-full resize-none text-sm leading-relaxed rounded-xl p-4 font-mono focus:outline-none focus:ring-2 ${
                  theme === 'dark'
                    ...
                    : 'bg-zinc-50 text-zinc-700 placeholder-zinc-400 border border-slate-300 focus:ring-blue-500/20'
                }`}
                spellCheck={false}
              />
