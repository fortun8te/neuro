...
export function StagePanel({ cycle, isRunning, isDarkMode: propDarkMode, viewStage }: StagePanelProps) {
  ...
  const tokenColor = isDark ? '#3b82f6' : '#374151';
    ...
    <div className="flex flex-col h-full min-h-0">
        ...
        <div className={`flex-shrink-0 border-b ${isDark ? 'border-zinc-800/60 bg-transparent' : 'border-zinc-200 bg-zinc-50'}`}>
          ...
          >
            ...
            <span className="text-[11px] font-semibold flex-1 truncate" style={{
              color: tokenInfo.isThinking ? (isDark ? '#3b82f6' : '#374151') : isDark ? '#71717a' : '#9ca3af'
            }}>
              {tokenInfo.isThinking ? 'Model Thinking' : 'Reasoning'}
            </span>
            <span className="text-[9px] px-1.5 py-0.5 rounded" style={{
              background: tokenInfo.isThinking ? 'rgba(234,88,12,0.15)' : isDark ? 'var(--glass-border-light)' : 'rgba(0,0,0,0.05)',
              color: tokenInfo.isThinking ? (isDark ? '#3b82f6' : '#374151') : isDark ? '#71717a' : '#9ca3af'
            }}>
            ...
            <svg
              ...
              style={{ color: tokenInfo.isThinking ? (isDark ? '#3b82f6' : '#374151') : isDark ? '#71717a' : '#9ca3af' }}
            >
              <path d="M6 9l6 6 6-6" />
            </svg>
          </button>
      ...
      {stageData.agentOutput ? (
        ...
        <div className={`flex-1 flex flex-col items-center justify-center gap-3 ${isDark ? 'bg-transparent' : 'bg-zinc-50'}`}>
          {currentStage === 'production' && !isRunning ? (
            ...
            <>
              <div className="w-24 h-0.5 rounded-full overflow-hidden" style={{ backgroundColor: isDark ? 'rgba(234,88,12,0.08)' : 'rgba(234,88,12,0.06)' }}>
              ...
              <TextShimmer className="text-[11px] [--shimmer-base:rgba(234,88,12,0.3)] [--shimmer-highlight:rgba(234,88,12,0.9)]" duration={1.8}>Processing</TextShimmer>
            </>
          ) : (
      ...
      {isActive && (
        <div className={`flex-shrink-0 flex items-center gap-3 px-5 py-2 border-t tabular-nums ${
          ...
          {/* Status + model */}
          {hasActivity ? (
            <TextShimmer className="text-[11px] font-medium [--shimmer-base:rgba(234,88,12,0.3)] [--shimmer-highlight:rgba(234,88,12,0.9)]" duration={1.8}>{tokenLabel}</TextShimmer>
          ) : (
          ...
          {tokenInfo.isModelLoading && tokenInfo.callStartTime && (
            <span className="text-[10px] tabular-nums" style={{ color: isDark ? '#3b82f6' : '#374151', opacity: 0.7 }}>
