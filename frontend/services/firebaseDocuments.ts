...
// Lazy-initialize Firestore to prevent errors when Firebase isn't configured
...
function getDb() {
  if (!db) {
    try {
      ...
      throw new Error('Firebase not configured. Set VITE_FIREBASE_* env vars to use cloud documents.');
    }
  }
  return db;
}
...
export async function uploadDocument(document: FirebaseDocument): Promise<void> {
  ...
  const docRef = doc(getDb(), DOCUMENTS_COLLECTION, document.id);
...
export async function downloadDocument(docId: string): Promise<FirebaseDocument | null> {
  ...
  const docRef = doc(getDb(), DOCUMENTS_COLLECTION, docId);
