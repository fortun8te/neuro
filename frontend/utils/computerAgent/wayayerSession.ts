/**
 * wayayerSession — Playwright browser session via Wayfayer server.
 *
 ...
 *
 * Wayfayer runs a real Chromium browser via Playwright.
...
/** Check whether Wayfayer is reachable. */
export async function isWayfayerHealthy(): Promise<boolean> {
  try {
...
export class WayfayerSession {
  ...
  private readonly RECOVERY_BACKOFF_BASE = 1000; // ms
  ...
  async open(url: string, signal?: AbortSignal): Promise<SessionScreenshot> {
    try {
      ...
      if (data.error) {
        console.warn(`[WayfayerSession] open returned error: ${data.error}`);
        this.healthy = false;
      ...
      console.warn(`[WayfayerSession] open unreachable: ${err instanceof Error ? err.message : String(err)}`);
      this.healthy = false;
      return { image_base64: '', url, title: '', width: 0, height: 0 };
    }
  }
  ...
  /** Attempt to recover an unhealthy session with exponential backoff. */
  private async attemptRecovery(signal?: AbortSignal): Promise<boolean> {
    if (this.recoveryAttempts >= this.MAX_RECOVERY_ATTEMPTS) {
      console.warn(`[WayfayerSession] Recovery failed: max attempts (${this.MAX_RECOVERY_ATTEMPTS}) exceeded`);
      return false;
    }

    // Exponential backoff: wait before retry
    const backoffMs = this.RECOVERY_BACKOFF_BASE * Math.pow(2, this.recoveryAttempts);
    const timeSinceLastAttempt = Date.now() - this.lastRecoveryTime;
    if (timeSinceLastAttempt < backoffMs) {
      const waitTime = backoffMs - timeSinceLastAttempt;
      console.log(`[WayfayerSession] Recovery backoff: waiting ${waitTime}ms (attempt ${this.recoveryAttempts + 1}/${this.MAX_RECOVERY_ATTEMPTS})`);
    ...
    console.log(`[WayfayerSession] Attempting recovery (${this.recoveryAttempts}/${this.MAX_RECOVERY_ATTEMPTS}) at ${new Date(this.lastRecoveryTime).toISOString()}`);

    try {
      // Close broken session if one exists
      if (this.sessionId) {
        console.log(`[WayfayerSession] Closing broken session: ${this.sessionId}`);
        await this.close();
      }

      // Check if Wayfayer server is reachable
      const isReachable = await isWayfayerHealthy();
      if (!isReachable) {
        console.warn(`[WayfayerSession] Recovery failed: Wayfayer server unreachable`);
        return false;
      }
      ...
      console.log(`[WayfayerSession] Recovery successful: session reset and healthy`);
      return true;
    } catch (err) {
      console.warn(`[WayfayerSession] Recovery attempt failed: ${err instanceof Error ? err.message : String(err)}`);
      return false;
    }
  }
  ...
  async navigate(url: string, signal?: AbortSignal): Promise<SessionActionResult> {
    if (!this.healthy) {
      console.warn('[WayfayerSession] navigate: session unhealthy, attempting recovery');
      const recovered = await this.attemptRecovery(signal);
      if (!recovered) {
        return { error: 'Session unhealthy and recovery failed' };
      }
    }
  ...
  async click(x: number, y: number, signal?: AbortSignal): Promise<SessionActionResult> {
    if (!this.healthy) {
      console.warn('[WayfayerSession] click: session unhealthy, attempting recovery');
      const recovered = await this.attemptRecovery(signal);
      if (!recovered) {
        return { error: 'Session unhealthy and recovery failed' };
      }
  ...
  async rightClick(x: number, y: number, signal?: AbortSignal): Promise<SessionActionResult> {
    if (!this.healthy) {
      console.warn('[WayfayerSession] rightClick: session unhealthy, attempting recovery');
      const recovered = await this.attemptRecovery(signal);
      if (!recovered) {
        return { error: 'Session unhealthy and recovery failed' };
      }
  ...
  async doubleClick(x: number, y: number, signal?: AbortSignal): Promise<SessionActionResult> {
    if (!this.healthy) {
      console.warn('[WayfayerSession] doubleClick: session unhealthy, attempting recovery');
      const recovered = await this.attemptRecovery(signal);
      if (!recovered) {
        return { error: 'Session unhealthy and recovery failed' };
      }
  ...
  async forward(signal?: AbortSignal): Promise<SessionActionResult> {
    if (!this.healthy) {
      console.warn('[WayfayerSession] forward: session unhealthy, attempting recovery');
      const recovered = await this.attemptRecovery(signal);
      if (!recovered) {
        return { error: 'Session unhealthy and recovery failed' };
      }
  ...
  async uploadFile(selector: string, filePath: string, signal?: AbortSignal): Promise<SessionActionResult> {
    if (!this.sessionId) throw new Error('WayfayerSession: no open session');
  ...
  async clickSelector(selector: string, signal?: AbortSignal): Promise<SessionActionResult> {
    if (!this.healthy) {
      console.warn('[WayfayerSession] clickSelector: session unhealthy, attempting recovery');
      const recovered = await this.attemptRecovery(signal);
      if (!recovered) {
        return { error: 'Session unhealthy and recovery failed' };
      }
  ...
  async type(
    ...
    if (!this.healthy) {
      console.warn('[WayfayerSession] type: session unhealthy, attempting recovery');
      const recovered = await this.attemptRecovery(signal);
      if (!recovered) {
        return { error: 'Session unhealthy and recovery failed' };
      }
  ...
  async screenshot(signal?: AbortSignal): Promise<SessionScreenshot> {
    ...
    try {
      ...
      console.warn(`[WayfayerSession] screenshot failed: ${err instanceof Error ? err.message : String(err)}`);
      this.healthy = false;
      return { image_base64: '', url: '', title: '', width: 0, height: 0 };
    }
  }
  ...
  async evalJs(js: string, signal?: AbortSignal): Promise<unknown> {
    ...
    if (result.error) throw new Error(`WayfayerSession.evalJs: ${result.error}`);
  ...
  async getElements(signal?: AbortSignal): Promise<{
    ...
    try {
      ...
      console.warn(`[WayfayerSession] getElements failed: ${err instanceof Error ? err.message : String(err)}`);
      return { elements: [], text: 'Failed to fetch interactive elements\n', count: 0 };
    }
  }
  ...
  private async actionSafe(
    ...
    try {
      ...
      console.warn(`[WayfayerSession] action failed (marking unhealthy): ${err instanceof Error ? err.message : String(err)}`);
      this.healthy = false;
  ...
  private async post<T>(
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        if (resp.ok) {
          controller.abort(); // BUG FIX: Cleanup AbortController before returning
          return resp.json() as Promise<T>;
        }

        // Retry on 5xx server errors
        if (resp.status >= 500 && attempt < MAX_RETRIES) {
          console.warn(`[WayfayerSession] ${path} returned ${resp.status}, retrying (${attempt + 1}/${MAX_RETRIES})...`);
          controller.abort(); // BUG FIX: Cleanup AbortController on retry
          await new Promise(r => setTimeout(r, 1000 * (attempt + 1)));
          continue;
        }

        controller.abort(); // BUG FIX: Cleanup AbortController before throwing
        throw new Error(`WayfayerSession ${path} HTTP ${resp.status}`);
      } catch (err) {
        clearTimeout(timer);
        controller.abort(); // BUG FIX: Cleanup AbortController in error handler
        ...
        if (attempt < MAX_RETRIES && err instanceof TypeError) {
          console.warn(`[WayfayerSession] ${path} network error, retrying (${attempt + 1}/${MAX_RETRIES})...`);
          await new Promise(r => setTimeout(r, 1000 * (attempt + 1)));
          continue;
        }
    ...
    throw new Error(`WayfayerSession ${path} failed after ${MAX_RETRIES + 1} attempts`);
  }
}
