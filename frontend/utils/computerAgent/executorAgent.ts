...
async function findElementByText(
  ...
  } catch (e) { /* AX lookup is best-effort — log at debug level only */
    if (typeof console !== 'undefined') console.debug('[executor] AX lookup failed:', e instanceof Error ? e.message : String(e));
  }
  return null;
}
...
/**
 * Convert browser-internal coordinates (0-1920 x 0-1080 Playwright viewport)
...
function readingPause(screenshotBase64Length: number): number {
  // Brief scan pause -- just enough to not look robotic
  const normalized = Math.min(1, screenshotBase64Length / 80_000);
  return 100 + normalized * 200; // 100-300ms (was 300-800)
}

/** Pause after clicking -- minimal reaction time. */
function clickPause(): number {
  return Math.random() * 100 + 50; // 50-150ms (was 100-300)
}

/** Pause after navigation -- wait for page to start loading. */
function navPause(): number {
  return Math.random() * 400 + 300; // 300-700ms (was 600-1400)
}

/** Pause before action -- brief "I see" moment. */
function thinkPause(): number {
  return Math.random() * 150 + 50; // 50-200ms (was 200-600)
}
...
function normaliseUrl(raw: string): string {
  ...
  // Intercept Google search URLs — Google blocks headless browsers with reCAPTCHA.
  // Redirect to SearXNG (no bot detection) or Bing as fallback.
  const googleSearchMatch = trimmed.match(/^https?:\/\/(?:www\.)?google\.com\/search\?(.+)/i);
  if (googleSearchMatch) {
    const params = new URLSearchParams(googleSearchMatch[1]);
    ...
    if (query) {
      if (_searxngAvailable) {
        probeSearxng();
        return `${INFRASTRUCTURE.searxngUrl}/search?q=${encodeURIComponent(query)}`;
      }
...
// Default viewport — matches session open request (1920×1080)
const DEFAULT_VIEWPORT: ViewportInfo = { width: 1920, height: 1080 };
...
BROWSER ACTIONS (fallback — use pixel coordinates when element list is unavailable):
  ...
  browser_right_click — {"type":"browser_right_click","x":450,"y":230,"reasoning":"open context menu on image"} Right-click to open browser context menu (save image, copy link, etc.)
  browser_double_click — {"type":"browser_double_click","x":450,"y":230,"reasoning":"select word"} Double-click for text selection, icon activation, etc.
  browser_type — {"type":"browser_type","text":"hello","selector":"input#search","reasoning":"type query"} "clear":true clears first
  browser_scroll — {"type":"browser_scroll","direction":"down","distance":300,"reasoning":"scroll to see more"} direction: "up"|"down"|"left"|"right". Optional: "x":640,"y":400 to scroll at a specific position (for overflow containers)
  browser_drag — {"type":"browser_drag","startX":200,"startY":300,"endX":500,"endY":300,"reasoning":"drag slider"} Human-like bezier drag with mousedown held. Use for sliders, drag-and-drop, selecting text ranges, resizing elements.
  ...
  browser_forward — {"type":"browser_forward","reasoning":"go forward in history"}
  browser_upload — {"type":"browser_upload","selector":"input[type=file]","filePath":"/path/to/file.pdf","reasoning":"upload document"} Upload a file to a file input element.
...
DESKTOP ACTIONS (for managing windows and apps):
  ...
  desktop_drag_window — {"type":"desktop_drag_window","app":"finder","toX":100,"toY":100,"reasoning":"move window"} Drag-repositions a window anywhere on the desktop.
  desktop_focus_window — {"type":"desktop_focus_window","app":"terminal","reasoning":"bring to front"}
  desktop_scroll — {"type":"desktop_scroll","app":"finder","direction":"down","amount":300,"reasoning":"scroll in window"} direction: "up"|"down"|"left"|"right". app: "chrome"|"finder"|"terminal"
...
FILE OPERATIONS (for saving findings, organizing data):
  ...
  browser_save_images — {"type":"browser_save_images","urls":[],"maxImages":20,"reasoning":"download all images visible on this page to workspace"} — use urls:[] to grab all imgs on page, or pass specific URLs
...
async function decideAction(
  ...
  const maybeImages = screenshotBase64 ? [screenshotBase64] : undefined;
  let response = '';
...
export interface ExecutorOptions {
  ...
  screenshotDims?: ViewportInfo;
  /**
   * Enable a brief "think before act" step before each decideAction() call.
   * The model considers what it sees and what to do next in 1-2 sentences.
   * Reduces hallucination on complex multi-step tasks. Default false.
   */
  think?: boolean;
  /**
   * Workspace/chat ID — when set, downloaded images (browser_save_images) are copied
   * into workspace/chats/{workspaceId}/images/ so they appear in the Neuro file panel.
   */
  workspaceId?: string;
}

// ─────────────────────────────────────────────────────────────
// Think-before-act (optional brief reasoning step)
// ─────────────────────────────────────────────────────────────

/**
 * Internal reasoning step: asks a fast model to consider the current screen
 * and goal in 1-2 sentences before decideAction() is called.
 * Silent — emits no UI events. Returns null on timeout or error.
 */
async function thinkStep(
  ...
  const prompt =
    `Looking at the current screen: ${stateDescription || '(unknown state)'}. ` +
    `Goal: ${instruction}. ` +
    `What is the most direct next action? Think in 1-2 sentences max.`;
  try {
    let note = '';
    // 5-second timeout — skip if slow
    const timeoutSignal = AbortSignal.timeout(5000);
    const combined = signal
      ? AbortSignal.any([signal, timeoutSignal])
      : timeoutSignal;
    await ollamaService.generateStream(prompt, 'You are a concise reasoning assistant. Reply in 1-2 sentences only.', {
      model: getModelForStage('fast'),
      temperature: 0.2,
      num_predict: 80,
      signal: combined,
      onChunk: (chunk: string) => { note += chunk; },
    });
    const cleaned = stripThink(note).trim();
    return cleaned.length > 0 ? cleaned : null;
  } catch {
    // Timeout or abort — skip silently
    return null;
  }
}
...
export async function runExecutorStep(
  ...
  // Last think-step note — prepended to executor context when think=true
  let thinkingNote = '';
  ...
  for (let iter = 1; ; iter++) {
    ...
    // But first, give the model one final chance to decide action / submit results
    if (iter > hardLimit) {
      onStatus?.(`[Executor] Hard iteration limit (${hardLimit}) reached. Getting final submission...`);
      lastFiveActions.push(`SYSTEM: FINAL — You have reached the iteration limit. Call the 'fail' action with your final findings or declare done.`);
      if (lastFiveActions.length > 6) lastFiveActions.shift();

      try {
        const finalShot = await (wayfarerSession?.isOpen && wayfarerSession?.healthy
          ? wayfarerSession.screenshot(signal)
          : captureDesktop(desktopEl));
        if (finalShot) {
          currentScreenshot = typeof finalShot === 'string' ? finalShot : finalShot.image_base64;
          onScreenshot?.(currentScreenshot);
        }
      } catch { /* ignore */ }

      // One final decideAction to let model submit structured result
      try {
        const finalAction = await decideAction(lastFiveActions.slice(-5), currentScreenshot, signal ?? AbortSignal.timeout(10000));
        if (finalAction) {
          const actionStr = `${iter}[FINAL]: ${finalAction.type}`;
          lastFiveActions.push(actionStr);
          onStatus?.(`[Executor] Final action: ${finalAction.type}`);
          // Record it but don't execute — just break after this
        }
      } catch (e) {
        onStatus?.(`[Executor] Final decision timeout (expected): ${e instanceof Error ? e.message : String(e)}`);
      }
    ...
    // 1. Capture screenshot — prefer Wayfarer session (real Playwright screenshot)
    //    over html2canvas DOM capture (which fails when desktop is off-screen/minimized)
    try {
      let shot = '';
      if (wayfarerSession?.isOpen && wayfarerSession.healthy) {
        try {
          const ssResult = await wayfarerSession.screenshot(signal);
          shot = ssResult.image_base64 || '';
        } catch { /* fall through to DOM capture */ }
      }
      if (!shot) {
        shot = await captureDesktop(desktopEl);
      }
      if (!shot) {
        const failMsg = '[Executor] FATAL: Screenshot capture failed (both Wayfarer and DOM empty) after 3 retries. Cannot continue without visual feedback.';
        onStatus?.(failMsg);
        findings.push('ERROR: Screenshot failure — no visual context available for next decision.');
        return {
          findings: findings.slice(0, 10),
          success: false,
          failureReason: 'Screenshot capture failed — unable to see desktop state',
          stepsDone: Math.min(iter, 40),
          error: failMsg,
        };
      }
    ...
    // Optional think-before-act: brief internal reasoning before deciding action
    if (think) {
      const note = await thinkStep(lastStateDescription, step.instruction, signal);
      if (note) thinkingNote = note;
    }

    const action = await decideAction(
      step,
      currentScreenshot,
      thinkingNote ? `${thinkingNote} ${lastStateDescription}` : lastStateDescription,
    ...
    // Emit human-readable action description for sidebar visibility
    {
      ...
      switch (action.type) {
        ...
        case 'browser_right_click':
          desc = `Right-clicking at (${action.x ?? '?'}, ${action.y ?? '?'})`;
          break;
        case 'browser_double_click':
          desc = `Double-clicking at (${action.x ?? '?'}, ${action.y ?? '?'})`;
          break;
        case 'browser_forward':
          desc = 'Going forward in history';
          break;
        case 'browser_upload':
          desc = `Uploading ${(action.filePath ?? '').split('/').pop() || 'file'}`;
          break;
        case 'browser_type':
        case 'type':
    ...
    if (isBrowserAction && !(wayfarerSession?.isOpen && wayfarerSession?.healthy)) {
      // Browser action requested but no working session — fail honestly AND loudly
      const failMsg = `Cannot execute ${action.type}: browser session not available`;
      onStatus?.(`[Executor] ⚠️  ${failMsg}`);
      lastFiveActions.push(`${iter}[SYSTEM]: Browser is NOT available (session unhealthy/closed). Cannot do ${action.type}.`);
      if (lastFiveActions.length > 5) lastFiveActions.shift();
      lastFiveActions.push(`${iter}[SYSTEM]: ONLY desktop_click, desktop_scroll, desktop_screenshot, and fail are available. Pick one or declare task impossible.`);
      if (lastFiveActions.length > 5) lastFiveActions.shift();
      actionsTaken.push(`${iter}[FAILED]: ${failMsg}`);
      continue;
    }
    ...
    if (isBrowserAction && wayfarerSession?.isOpen && wayfarerSession.healthy) {
      ...
      try {
        switch (action.type) {
          ...
          case 'browser_right_click': {
            const rcX = action.x ?? 640;
            const rcY = action.y ?? 400;
            const rcScaled = scaleCoords(rcX, rcY, screenshotDims, browserViewport);
            const rcDc = browserToDesktopCoords(rcScaled.x, rcScaled.y, desktopEl);
            desktopBus.emit({ type: 'ai_cursor_move', x: rcDc.x, y: rcDc.y, state: 'acting' });
            await wayfarerSession.rightClick(rcScaled.x, rcScaled.y, signal);
            desktopBus.emit({ type: 'ai_cursor_click', x: rcDc.x, y: rcDc.y });
            await sleep(clickPause());
            break;
          }
          case 'browser_type':
          ...
          case 'browser_scroll': {
            ...
            // Emit scroll indicator at the scroll position (center of viewport if no coords given)
            const scrollBX = action.x ?? 640;
            const scrollBY = action.y ?? 400;
            const scrollDc = browserToDesktopCoords(scrollBX, scrollBY, desktopEl);
            desktopBus.emit({ type: 'ai_cursor_move', x: scrollDc.x, y: scrollDc.y, state: 'acting' });
            desktopBus.emit({ type: 'ai_cursor_scroll', x: scrollDc.x, y: scrollDc.y, direction: scrollDir as 'up' | 'down' | 'left' | 'right' });
          ...
          case 'browser_drag': {
            ...
            // Emit drag trail for visual feedback
            desktopBus.emit({ type: 'ai_cursor_drag_trail', fromX: dragStartDc.x, fromY: dragStartDc.y, toX: dragEndDc.x, toY: dragEndDc.y });
            break;
          }
          case 'browser_shortcut':
          ...
          case 'browser_back':
            ...
            break;
          case 'browser_forward':
            await wayfarerSession.forward(signal);
            await sleep(navPause());
            break;
          case 'browser_double_click': {
            const dcX = action.x ?? 640;
            const dcY = action.y ?? 400;
            const dcScaled = scaleCoords(dcX, dcY, screenshotDims, browserViewport);
            const dcDc = browserToDesktopCoords(dcScaled.x, dcScaled.y, desktopEl);
            desktopBus.emit({ type: 'ai_cursor_move', x: dcDc.x, y: dcDc.y, state: 'acting' });
            await wayfarerSession.doubleClick(dcScaled.x, dcScaled.y, signal);
            desktopBus.emit({ type: 'ai_cursor_click', x: dcDc.x, y: dcDc.y });
            await sleep(clickPause());
            break;
          }
          case 'browser_upload': {
            const uploadSelector = action.selector || 'input[type=file]';
            const uploadPath = action.filePath || '';
            if (!uploadPath) {
              onStatus?.('[Executor] browser_upload: no filePath provided');
              break;
            }
            const uploadResult = await wayfarerSession.uploadFile(uploadSelector, uploadPath, signal);
            onStatus?.(`[Executor] Uploaded file: ${uploadPath}`);
            findings.push(`Uploaded ${uploadPath} to ${uploadSelector}`);
            break;
          }
          case 'browser_reload':
          ...
          case 'browser_save_images': {
            const imgUrls: string[] = Array.isArray(action.urls) ? (action.urls as string[]) : [];
            const maxImgs: number = typeof action.maxImages === 'number' ? action.maxImages : 20;
            const saveResult = await wayfarerSession.saveImages(imgUrls, maxImgs, signal);
            if (saveResult.ok && saveResult.count > 0) {
              onStatus?.(`[Executor] Saved ${saveResult.count} image(s): ${saveResult.saved.slice(0, 5).join(', ')}`);
              findings.push(`Downloaded ${saveResult.count} image(s) to workspace: ${saveResult.saved.slice(0, 5).join(', ')}`);
              // Copy files into Neuro workspace so they appear in the file panel
              if (workspaceId && wayfarerSession.id) {
                try {
                  const sessionDir = `/tmp/wayfarer_downloads/${wayfarerSession.id}`;
                  const destDir = `$HOME/Downloads/nomads/workspace/chats/${workspaceId}/images`;
                  await fetch('/api/shell', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      command: `mkdir -p "${destDir}" && cp ${saveResult.saved.map(f => `"${sessionDir}/${f}"`).join(' ')} "${destDir}/"`,
                      timeout: 10000,
                    }),
                  });
                  onStatus?.(`[Executor] Images copied to workspace/images/`);
                } catch { /* best-effort */ }
              }
            } else {
              onStatus?.('[Executor] browser_save_images: no images saved (page may have no images or Wayfarer unavailable)');
            }
            break;
          }
          default:
        ...
        if (isSessionDead && wayfarerSession) {
          ...
          try {
            ...
            // Session is permanently dead — fail immediately instead of looping 20+ more iterations
            const failMsg = '[Executor] FATAL: Browser session dead and recovery failed. Cannot continue.';
            onStatus?.(failMsg);
            findings.push('ERROR: Browser connection lost and could not be recovered.');
            return {
              findings: findings.slice(0, 10),
              success: false,
              failureReason: 'Browser session crashed and unrecoverable',
              stepsDone: Math.min(iter, 40),
              error: failMsg,
            };
          }
