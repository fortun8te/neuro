# Wayfayer File Transfer Architecture
...
**Current Issue**: Wayfayer downloads/scrapes files into the Docker container at `/tmp/wayfayer_downloads/{session_id}/` but there's no mechanism for the client (web UI running on host) to retrieve those files.

**Symptoms**:
- Files downloaded during research are lost after Wayfayer processes them
...
**Root Cause**: One-way request/response pattern. Wayfayer receives requests, sends back JSON responses, but has no way to serve binary files to remote clients.
...
### Overview
Add a **session-scoped file transfer endpoint** to Wayfayer API that:
1. Stores downloaded files in `/tmp/wayfayer_downloads/{session_id}/` during scraping
...
4. Wayfayer streams file back as binary blob
...
- Clean separation: Wayfayer handles download, client handles analysis
...
**File**: `/Users/mk/Downloads/nomads/wayfayer/wayfayer_server.py`
...
Add these methods to the existing `WayfayerClient` class:
...
```bash
# Start Wayfayer locally
python -m uvicorn wayfayer_server:app --port 8889
...
```typescript
// In React test suite
describe('WayfayerClient file downloads', () => {
  it('should list session files', async () => {
    const client = new WayfayerClient('http://localhost:8889', 'test-key');
...
- [ ] Update `wayfayer_server.py` with all 4 endpoints
...
- [ ] Test on remote Wayfayer instance
- [ ] Add error handling for network failures
- [ ] Document new endpoints in README
- [ ] Add user-facing UI for file downloads
- [ ] Monitor `/tmp/wayfayer_downloads/` disk usage
...
Add logging for:
1. File downloads: `[Wayfayer] Downloaded {filename} ({size} KB) for session {session_id}`
2. File cleanup: `[Wayfayer] Cleaned up old downloads: {session_id} ({count} files)`
3. Errors: `[Wayfayer] File download error: {session_id}/{filename}: {error}`
