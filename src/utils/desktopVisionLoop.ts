...
export async function runDesktopVisionLoop(
  ...
  const { signal, onStep, onScreenshot, maxSteps = 15 } = options;
  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    // 5. Wait for React to re-render before next screenshot
    await new Promise<void>((resolve) => setTimeout(resolve, 700));
  }

  return `Stopped after ${maxSteps} steps. History: ${history.slice(-3).join('; ')}`;
}
