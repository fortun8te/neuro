...
import { getModelForStage, getPlannerModel, getExecutorModel, getThinkMode, getImageModel } from './modelConfig';
...
function buildTools(onEvent?: AgentEngineCallback): ToolDef[] {
  return [
    {
      name: 'web_search',
      ...
      execute: async (params, signal) => {
        try {
          ...
          // Primary search
          const results = await wayfarerService.research(query, max, signal);
          ...
          if (depth === 'thorough') {
            ...
            if (variations.length > 0) {
              ...
              const followUps = await Promise.all(
                variations.map(vq => wayfarerService.research(vq, perVariation, signal))
              );
          ...
          // Return a generic "no results" — NOT a tool error string.
          // If the model sees "Search failed: fetch failed" it reasons about its config and leaks internals.
          // Returning a search-result-style empty state keeps it in "researcher mode" not "debugger mode".
    ...
    },
    {
      ...
      execute: async (params, signal) => {
        try {
          ...
          const result = await wayfarerService.batchCrawl(urls, 10, signal);
    ...
    },
    {
      ...
      execute: async (params, signal) => {
        try {
          ...
          const subagentModel = getModelForStage('fast'); // 2b — fast enough for most research tasks
    ...
    },
    {
      ...
      execute: async (params, signal) => {
        try {
          ...
          // Map depth param to source count, with expanded 'deep' range
          let sourceCount: number;
          if (depthParam === 'quick') sourceCount = 3;
          else if (depthParam === 'deep') sourceCount = 20; // was 15, now more thorough
          else if (depthParam === 'very deep' || depthParam === 'extremely deep') sourceCount = 35; // super comprehensive
          else if (depthParam === 'super' || depthParam === 'ultra') sourceCount = 40; // maximum depth
          else sourceCount = 8; // normal

          const queries = [
            `${topic} ${angle}`.trim(),
            `${topic} overview statistics data`,
            `${topic} expert analysis research`,
            depth !== 'quick' ? `${topic} case studies examples` : '',
            depth !== 'quick' ? `${topic} trends 2024 2025` : '',
          ].filter(Boolean).slice(0, depth === 'quick' ? 2 : depth === 'deep' ? 5 : 3);
...
**${totalPoints} data points** across **${spec.series.length} series** — visualized as a ${spec.type} chart.`;
    ...
    {
      name: 'write_content',
      description: 'Write polished content: blog posts, emails, ad copy, social captions, product descriptions, scripts, reports.',
      parameters: {
      ...
      execute: async (params, signal) => {
        try {
          ...
          await ollamaService.generateStream(
            ...
            { model: getModelForStage('production'), temperature: 0.7, num_predict: 1500, signal, onChunk: (c: string) => { content += c; } }
          );
...
| "write copy" / "blog post" / "email" / "ad copy" | write_content | Auto-saves to file |
...
function looksLikeFakeToolNarration(text: string, stepHadToolCall: boolean): boolean {
  if (stepHadToolCall) return false; // real tool was called, this is fine
...
function looksLikeModelIdentityClaim(text: string): boolean {
  const t = text.toLowerCase();
  return (
    // Claims of being Qwen
    /\b(i'?m\s+)?(actually\s+)?qwen\b/.test(t) ||
    /\bunfortunately.*qwen\b/.test(t) ||
    /\bqwen.*model\b/.test(t) ||
    // Claims of being Claude
    /\b(i'?m\s+)?(actually\s+)?claude\b/.test(t) ||
    /\bunfortunately.*claude\b/.test(t) ||
    // Claims of being ChatGPT or GPT
    /\b(i'?m\s+)?(actually\s+)?(chat)?gpt\b/.test(t) ||
    /\bunfortunately.*gpt\b/.test(t) ||
    // Generic "I'm a language model" claims
    /\bi'?m\s+a\s+language\s+model\b/.test(t) ||
    /\bi'?m\s+(?:really|actually|just|simply)\s+\w+\s+model\b/.test(t) ||
    // "Under the hood" style claims
    /\bunder\s+the\s+hood.*?(qwen|claude|gpt)\b/.test(t) ||
    /\bat\s+(?:my|the)\s+core.*?(qwen|claude|gpt)\b/.test(t)
  );
}
...
type TaskType = 'research' | 'code' | 'analyze' | 'create' | 'file' | 'computer' | 'agents' | 'memory' | 'dataviz' | 'general';
...
const TASK_TOOLS: Record<TaskType, string[]> = {
  ...
  code:     ['shell_exec', 'file_read', 'file_write', 'apply_patch', 'run_code', 'file_find', 'web_search'],
...
function classifyTaskType(msg: string): TaskType {
  ...
  // Code / scripting
  if (/\b(write.*code|write.*script|function|debug|fix.*bug|refactor|implement|program|python|javascript|typescript|bash|shell|command|terminal|npm|pip|git)\b/.test(t)) return 'code';
...
function compactOldToolResults(entries: string[]): string[] {
  ...
  const KEEP_PREFIX = 120;          // keep first 120 chars of result (enough for the gist)
  ...
  return entries.map((entry, idx) => {
    ...
    const resultMarkerIdx = entry.indexOf('): ');
    if (resultMarkerIdx === -1 || resultMarkerIdx + 3 + KEEP_PREFIX >= entry.length) return entry;
    const prefixEnd = resultMarkerIdx + 3 + KEEP_PREFIX;
    return entry.slice(0, prefixEnd) + COMPACT_SUFFIX;
  });
}
...
export async function runAgentLoop(
  ...
  const timeLoc = extractTimeLocation(userMessage);   // null = no match, string = location (empty = local)
  const weatherLoc = extractWeatherLocation(userMessage); // null = no match
  if (timeLoc !== null) {
  ...
  const simple = isSimpleQuestion(userMessage);
  const complex = !simple && isComplexTask(userMessage);
...
- You're a teenager talking to your friend Michael. Be real.`;
    ...
    const neuroUp = await isNeuroAvailable();
    if (neuroUp) {
      const neuroReply = await askNeuroIdentity(userMessage, conversationHistory, {
        signal,
        onChunk: (c: string) => {
          response += c;
      ...
      // NEURO failed — fall through to Qwen fast-path
      response = '';
    }
  ...
  if (isIdentityQuestion(userMessage) && await isNeuroAvailable()) {
    ...
    let response = '';
    const neuroResponse = await askNeuroIdentity(
      userMessage,
      conversationHistory,
      {
        signal,
    ...
    // NEURO failed — fall through to normal pipeline
  }

  // ── Phase 2: Tool Router ──
  // Priority: regex quick-select → Qwen LLM router → regex task-pruning fallback
  const allTools = buildTools(onEvent);
  const toolRouterStart = Date.now();
  let tools: ToolDef[];
  const quickTools = quickSelectTools(userMessage);
  ...
  if (quickTools) {
    ...
    try {
      ...
      const selection = await selectTools({ goal: userMessage, userMessage }, signal);
  ...
  try {
    ...
    semanticRoutingResult = await neuroEnhancedRouter.route(
      userMessage,
      toolNames,
      conversationHistory,
      0 // fresh token count
  ...
  let routed = routeToModel(userMessage);
  let model = modelOverride || routed.model;
  let temperature = tempOverride ?? (routed.tier === 'tiny' ? 0.8 : routed.tier === 'small' ? 0.7 : 0.6);
  onEvent({ type: 'routing', timestamp: Date.now(), routing: { phase: 'model-select', decision: `${routed.tier} → ${model}` } });

  // ── Image auto-routing ──
  // When the conversation includes images, upgrade to a vision-capable model.
  // Only applies when the caller did NOT explicitly request a specific model.
  if (!modelOverride && hasImageContent(userMessage, conversationHistory)) {
    model = getImageModel();
  }
...
${toCompress}`,
  ...
  const [soulContext, skillContext] = await Promise.all([
    ...
    loadRelevantSkills(userMessage).catch(() => ''),
  ]);
  ...
  const subagentPlan = await planSubagents(userMessage, conversationHistory);
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    // Step limit warning — when approaching max, tell user
    if (step === maxSteps - 2) {
    ...
    if (getInjectedMessages) {
      ...
      for (const msg of injected) {
        ...
        if (!modelOverride) {
          ...
          const tierOrder: ModelTier[] = ['tiny', 'small', 'medium', 'large', 'xlarge'];
    ...
    if (!(step === 0 && thinkingStartedBeforeLoop)) {
      onEvent({ type: 'thinking_start', step, timestamp: Date.now(), model });
    }
    ...
    async function callLLMWithRetry(): Promise<void> {
      ...
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        try {
          // Dynamic num_predict based on request type and complexity
          const msgLower = currentContext.toLowerCase();
          const isDetailRequest = /\b(detail|explain|analyze|research|compare|describe|elaborate|comprehensive|thorough|evaluate|report|write|create|generate|story|essay|article)\b/i.test(msgLower);
          const isSimpleRequest = /^(hi|hey|yo|thanks|ok|cool|nice|bye|gm|gn)\s*[!?.]*$/i.test(msgLower.trim());
          const dynamicNumPredict = isSimpleRequest ? 400 : isDetailRequest ? 2000 : 1500;
          ...
          await ollamaService.generateStream(
            ...
            systemPrompt,
            {
              model,
              temperature,
              // No thinking tokens — they add 10-30s of latency with minimal benefit.
              // The model reasons inline via its tool calls and response text.
              think: false,
              // Dynamic num_predict based on request complexity with tier-based cap
              // Simple requests (400), detail requests (2000), default (1500)
              num_predict: Math.min(dynamicNumPredict, routed.tier === 'tiny' ? 400 : routed.tier === 'small' ? 1500 : 2000),
              signal: stepSignal,
          ...
          onEvent({ type: 'error', error: `LLM error (attempt ${attempt + 1}/${MAX_ATTEMPTS}), retrying in ${Math.round(waitMs / 1000)}s...`, step, timestamp: Date.now() });
    ...
    try {
      ...
      contextEntries.push(`System: LLM error at step ${step} (all retries exhausted): ${errMsg}. Moving to next step.`);
    ...
    if (!toolCallParsed) {
      ...
      if (looksLikeModelIdentityClaim(llmResponse) && formatRetryCount < MAX_FORMAT_RETRIES) {
        formatRetryCount++;
        contextEntries.push(
      ...
      // If the model faked a tool call (said "just checked..." without calling any tool), force it to actually call one
      if (looksLikeFakeToolNarration(llmResponse, steps.some(s => s.toolCall)) && formatRetryCount < MAX_FORMAT_RETRIES) {
        formatRetryCount++;
        contextEntries.push(
          `Assistant: ${llmResponse}\n\nSystem: You claimed to have searched/checked something but did NOT call any tool. ` +
          `Your response was answered from memory, which is not allowed for factual questions. ` +
        ...
        if (useXlam) {
          try {
            ...
            if (xlamCalls.length > 0) {
              ...
              if (tool) {
                onEvent({ type: 'tool_start', toolCall: { id: `xlam-${Date.now()}`, name: rescued.name, args: rescued.args }, step, timestamp: Date.now() });
                const result = await tool.execute(rescued.args, signal);
                onEvent({ type: 'tool_done', toolCall: { id: `xlam-${Date.now()}`, name: rescued.name, args: rescued.args, result }, step, timestamp: Date.now() });
    ...
    if (toolCallParsed.name === 'done') {
      const rawSummary = String(toolCallParsed.args.summary || llmResponse);
    ...
    // ── Parallel execution: if multiple tool blocks detected, run them simultaneously ──
    const allParsed = parseAllToolCalls(llmResponse);
    const parallelCandidates = allParsed.filter(tc => PARALLEL_SAFE_TOOLS.has(tc.name));
    ...
    if (parallelCandidates.length >= 2) {
      ...
      onEvent({ type: 'routing', timestamp: Date.now(), routing: { phase: 'parallel', decision: `Parallel execution: ${parallelCandidates.map(t => t.name).join(' + ')}`, tools: parallelCandidates.map(t => t.name), durationMs: 0 } });

      // Execute all in parallel
      const parallelResults = await Promise.all(parallelToolCalls.map(async (tc) => {
        const toolDef = tools.find(t => t.name === tc.name);
        if (!toolDef) {
          tc.status = 'error';
          return { tc, result: { success: false, output: `Unknown tool: ${tc.name}` } as ToolResult };
        }
        try {
          const result = await toolDef.execute(tc.args, signal);
          if (signal?.aborted) { tc.status = 'error'; return { tc, result: { success: false, output: 'Aborted' } as ToolResult }; }
          ...
          tc.status = 'error';
          tc.result = { success: false, output: `Error: ${err instanceof Error ? err.message : err}` };
          tc.completedAt = Date.now();
          return { tc, result: tc.result };
        }
      }));
      ...
      contextEntries.push(`Assistant: ${thinking}\n\n[Parallel tool execution: ${parallelCandidates.map(t => t.name).join(', ')}]\n\n${aggregatedOutput}`);
    ...
    // Context window management: truncate large results
    let resultOutput: string;
    const CONTEXT_RESULT_LIMIT = 20000;

    if (result.output.length > CONTEXT_RESULT_LIMIT) {
