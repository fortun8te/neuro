...
const lc = (t: LT) => t === 'prompt' ? '#4ade80' : t === 'error' ? '#ef4444' : t === 'success' ? '#22c55e' : t === 'system' ? 'rgba(255,255,255,0.28)' : 'rgba(255,255,255,0.60)';
...
const WELCOME: TLine[] = [
  { id: 'w1', type: 'system', text: 'Nomad Terminal  v1.0.0' },
  ...
  return (
    <motion.div ref={windowRef} initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.92 }} transition={{ duration: 0.18, ease: [0.25, 0.46, 0.45, 0.94] }} onClick={() => inputRef.current?.focus()} onMouseDownCapture={onFocus} style={{ position: 'absolute', ...(pos ? { left: pos.x, top: pos.y, transform: 'none' } : { left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }), width: 640, height: 380, maxWidth: 'calc(100% - 8px)', maxHeight: 'calc(100% - 8px)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', zIndex: zIndex ?? 220, pointerEvents: 'auto', background: '#0a0a0c', boxShadow: '0 4px 24px rgba(0,0,0,0.4)', border: 'none', fontFamily: 'ui-monospace,"SF Mono","Fira Mono","Cascadia Code","JetBrains Mono",monospace', userSelect: 'none' }}>
      ...
      <div onMouseDown={onTitleBarMouseDown} style={{ height: 36, display: 'flex', alignItems: 'center', paddingLeft: 14, paddingRight: 14, background: '#1c1c1e', borderBottom: '1px solid rgba(255,255,255,0.04)', flexShrink: 0, cursor: isDragging ? 'grabbing' : 'grab', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.45)', letterSpacing: 0.2 }}>Terminal</div>
      </div>
      {/* Output */}
      ...
      {/* Input */}
      <div style={{ height: 38, borderTop: '1px solid rgba(255,255,255,0.04)', background: '#0d0d10', display: 'flex', alignItems: 'center', padding: '0 14px', flexShrink: 0 }}>
        <span style={{ fontSize: 12.5, color: '#4ade80', whiteSpace: 'nowrap', flexShrink: 0, userSelect: 'none' }}>{PS}</span>
        <div style={{ position: 'relative', flex: 1, display: 'flex', alignItems: 'center' }}>
          <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={onKey} spellCheck={false} autoComplete="off" autoCorrect="off" autoCapitalize="off" style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', fontSize: 12.5, color: 'rgba(255,255,255,0.88)', fontFamily: 'inherit', caretColor: 'transparent', width: '100%' }} />
