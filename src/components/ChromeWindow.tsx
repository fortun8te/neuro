...
function LockIcon({ locked = true }: { locked?: boolean }) {
  return (
    <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={locked ? 'rgba(134,239,172,0.7)' : 'rgba(255,255,255,0.25)'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
function FaviconDot({ favicon, size = 13 }: { favicon?: string; size?: number }) {
  if (!favicon) return <span style={{ display: 'inline-block', width: size, height: size, borderRadius: '50%', background: 'rgba(255,255,255,0.15)', flexShrink: 0 }} />;
...
function NavButton({ children, onClick, disabled, title }: { children: React.ReactNode; onClick: () => void; disabled: boolean; title?: string }) {
  return (
    <button onClick={onClick} disabled={disabled} title={title}
      style={{ width: 28, height: 28, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: 'none', cursor: disabled ? 'default' : 'pointer', color: disabled ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.45)', transition: 'color 0.15s, background 0.15s', flexShrink: 0 }}
  ...
  return (
    <motion.div layout initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: 'auto' }} exit={{ opacity: 0, width: 0 }} transition={spring}
      ...
      style={{ height: 34, maxWidth: 200, minWidth: 60, display: 'flex', alignItems: 'center', gap: 6, padding: '0 8px 0 10px', borderRadius: '7px 7px 0 0', cursor: 'pointer', flexShrink: 1, overflow: 'hidden', position: 'relative',
        background: tab.isActive ? 'rgba(255,255,255,0.07)' : hovered ? 'rgba(255,255,255,0.035)' : 'transparent',
      ...
      <span style={{ flex: 1, fontSize: 11, fontWeight: tab.isActive ? 500 : 400, color: tab.isActive ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.35)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', userSelect: 'none' }}>{title}</span>
      <button onClick={onClose}
        style={{ width: 16, height: 16, borderRadius: 3, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: hovered || tab.isActive ? 'rgba(255,255,255,0.06)' : 'transparent', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.3)', opacity: hovered || tab.isActive ? 1 : 0, transition: 'opacity 0.15s, background 0.15s' }}
...
export const ChromeWindow = forwardRef<ChromeWindowHandle, ChromeWindowProps>(function ChromeWindow(
  ...
  const [tabs, setTabs] = useState<ChromeTab[]>(() => {
    ...
    if (last) return [makeTab({ url: last, title: cleanUrl(last), history: [last], historyIndex: 0 })];
    return [makeTab()];
  });
  ...
  return (
    <motion.div ref={windowRef} data-chrome-window tabIndex={-1}
      ...
      style={{
        position: 'absolute',
        ...(pos ? { left: pos.x, top: pos.y, transform: 'none' } : { left: '50%', top: '40%', transform: 'translate(-50%, -50%)' }),
        width: 780, height: 460, maxWidth: 'calc(100% - 8px)', maxHeight: 'calc(100% - 8px)',
        display: 'flex', flexDirection: 'column', borderRadius: 12, overflow: 'hidden',
        background: '#0a0a0c', boxShadow: '0 4px 24px rgba(0,0,0,0.4)',
      ...
      <div onMouseDown={onTabBarMouseDown}
        style={{ height: 36, display: 'flex', alignItems: 'flex-end', background: '#1c1c1e', borderBottom: '1px solid rgba(255,255,255,0.04)', paddingLeft: 40, paddingRight: 8, position: 'relative', cursor: isDraggingWindow ? 'grabbing' : 'grab' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 0, flex: 1, minWidth: 0, overflow: 'hidden' }}>
          ...
          <motion.button onClick={() => openTab()} whileHover={{ scale: 1.1, color: 'rgba(255,255,255,0.6)' }} whileTap={{ scale: 0.92 }}
            style={{ width: 28, height: 28, borderRadius: 6, marginBottom: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.25)', flexShrink: 0, marginLeft: 4 }}>
            <PlusIcon />
          </motion.button>
        </div>
          ...
          <div style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', display: 'flex', alignItems: 'center', gap: 6 }}>
            <button onClick={() => { if (humanControl) releaseControl(); else { humanActionsRef.current = []; setHumanControl(true); } }}
              style={{ padding: '1px 6px', borderRadius: 4, fontSize: 8, fontWeight: 600, letterSpacing: '0.04em', lineHeight: '14px', cursor: 'pointer', whiteSpace: 'nowrap', transition: 'all 0.15s',
                border: humanControl ? '1px solid rgba(251,191,36,0.35)' : '1px solid rgba(255,255,255,0.10)',
                background: humanControl ? 'rgba(251,191,36,0.12)' : 'rgba(255,255,255,0.04)',
                color: humanControl ? 'rgba(251,191,36,0.9)' : 'rgba(255,255,255,0.35)',
              }}
      ...
      {/* ── TOOLBAR (40px) ── */}
      <div style={{ height: 40, display: 'flex', alignItems: 'center', gap: 8, padding: '0 12px', background: '#1c1c1e' }}>
        {/* A11y toggle */}
        <button onClick={() => setA11yOn(v => !v)} title={a11yOn ? 'Hide accessibility overlay' : 'Show accessibility overlay'}
          style={{ width: 28, height: 28, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 9, fontWeight: 700, fontFamily: 'ui-monospace, monospace', letterSpacing: '0.02em', transition: 'all 0.15s', flexShrink: 0,
            ...
            color: a11yOn ? '#a855f7' : 'rgba(255,255,255,0.35)',
          }}
          ...
          onMouseLeave={e => { if (!a11yOn) { e.currentTarget.style.color = 'rgba(255,255,255,0.35)'; e.currentTarget.style.background = 'none'; } }}>
          A11y
        </button>
        ...
        <form onSubmit={onUrlSubmit} style={{ flex: 1, display: 'flex' }}>
          <div onClick={onUrlFocus}
            style={{
              ...
              background: editingUrl ? 'rgba(255,255,255,0.07)' : 'rgba(255,255,255,0.04)',
              border: editingUrl ? '1px solid rgba(99,155,255,0.4)' : isNavigating ? '1px solid rgba(59,130,246,0.35)' : '1px solid rgba(255,255,255,0.06)',
            ...
            {editingUrl ? (
              <input ref={urlRef} value={urlInput} onChange={e => setUrlInput(e.target.value)} onBlur={onUrlBlur} onKeyDown={onUrlKey} autoFocus
                style={{ flex: 1, background: 'none', border: 'none', outline: 'none', fontSize: 12, color: 'rgba(255,255,255,0.8)', caretColor: '#3b82f6', fontFamily: 'ui-monospace, monospace', pointerEvents: 'auto' }} />
            ) : (
              <span style={{ flex: 1, fontSize: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', userSelect: 'none', transition: 'color 0.2s',
                color: showBlank ? 'rgba(255,255,255,0.2)' : isNavigating ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.6)' }}>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', minHeight: 0 }}>
        ...
        {streamId && hasFrame ? (
          <div style={{ width: '100%', height: '100%', position: 'relative', background: '#0a0a0c', overflow: 'hidden', touchAction: 'none' }}>
            ...
            <div style={{ position: 'absolute', bottom: 6, right: 8, fontSize: 9, background: 'rgba(0,0,0,0.3)', borderRadius: 4, padding: '1px 5px', pointerEvents: 'none', zIndex: 7,
              color: streamOk ? 'rgba(40,200,64,0.5)' : 'rgba(255,255,255,0.15)' }}>
          ...
          <div style={{ width: '100%', height: '100%', background: '#0a0a0c', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <form onSubmit={e => { e.preventDefault(); const fd = new FormData(e.currentTarget); const q = (fd.get('q') as string || '').trim(); if (!q) return; const isUrl = /^https?:\/\//.test(q) || /^[a-z0-9-]+\.[a-z]{2,}/.test(q); navigateTo(isUrl ? (q.startsWith('http') ? q : 'https://' + q) : `${INFRASTRUCTURE.searxngUrl}/search?q=${encodeURIComponent(q)}`); }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 99, padding: '0 16px', height: 44 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" /></svg>
                <input name="q" autoFocus placeholder="Search or enter URL" style={{ flex: 1, background: 'none', border: 'none', outline: 'none', fontSize: 13, color: 'rgba(255,255,255,0.7)', caretColor: '#3b82f6' }} />
              </div>
            </form>
          </div>
        ) : (
